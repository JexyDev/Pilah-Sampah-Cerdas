...
2. **Konfirmasi definisi tugas role Petugas (Residu)** berdasarkan apa yang sudah saya jelaskan sebelumnya: tugas utamanya adalah **membuang/mengangkut sampah dari tong yang penuh** (fokus pada sampah residu/tong yang sudah penuh kapasitasnya), menggunakan modul Peta & Monitoring untuk mengetahui lokasi dan status tong, lalu melakukan aksi "Kosongkan Tong" di lapangan.
3. **Rangkum fitur apa saja yang seharusnya dimiliki Petugas** berdasarkan QC + konteks tugas di atas:
   ...
   - Beranda dengan ringkasan tugas: jumlah tong penuh di area tugasnya (poin 4.8), role badge "Petugas" (poin 4.2), alert notifikasi tong penuh >90% (poin 4.13, terkait BUG-004 & poin 11.12).
   - Akses penuh ke modul **Peta & Monitoring** (kategori 7) — termasuk hak untuk menekan tombol "Kosongkan Tong" (berbeda dengan Mahasiswa yang perlu dikonfirmasi apakah read-only atau tidak, sesuai prompt sebelumnya — Petugas **PASTI punya hak eksekusi**, ini yang membedakan).
...
**B. Navigasi Utama**
- Tab yang tampil untuk Petugas: Beranda, Profil, dan akses ke Peta & Monitoring (baik lewat tab tersendiri atau lewat Beranda — sebutkan pendekatan yang dipakai dan alasannya). Tab Riwayat/Poin/Scan **TIDAK berlaku** untuk Petugas kecuali ada instruksi lain dari saya — cek dan konfirmasi dulu jika ternyata Petugas juga butuh riwayat aktivitas "tong yang sudah dikosongkan" sebagai bentuk riwayat tersendiri (beda dari riwayat scan Warga/Mahasiswa).

**C. Beranda/Dashboard Khusus Petugas**
- Role badge "Petugas" (poin 4.2).
- Ringkasan jumlah tong penuh di area tugas (poin 4.8) — jelaskan bagaimana "area tugas" ditentukan (apakah berdasarkan wilayah/Kelurahan yang di-assign, radius dari lokasi Petugas, atau seluruh tong tanpa filter wilayah — cek/konfirmasi ke saya jika belum jelas di data).
- Alert/notifikasi saat ada tong dengan kapasitas >90% (poin 4.13, 11.12, terkait BUG-004) — pastikan notifikasi ini benar-benar ter-trigger dari backend (bukan cuma UI kosong).
- Notifikasi bisa di-tap untuk langsung ke lokasi tong terkait di Peta (mirip pola poin 4.14 untuk role lain, disesuaikan konteks Petugas).

**D. Peta & Monitoring (Fitur Utama Petugas)**
- Full-screen map, posisi GPS Petugas (titik biru), marker tong hijau (aman)/merah berkedip (penuh) (poin 7.1-7.4).
- Auto-refresh data peta setiap 30 detik (poin 7.5), zoom/pan (poin 7.6).
- Tap marker merah → Bottom Sheet dengan info Tong ID, alamat, persentase kapasitas (poin 7.7-7.10).
- Tombol "Kosongkan Tong" dengan dialog konfirmasi sebelum eksekusi (poin 7.11-7.12).
- Setelah konfirmasi: hit API reset kapasitas tong, marker berubah dari merah ke hijau secara real-time (poin 7.13).
...
- JWT expired saat sedang proses kosongkan tong di lapangan — pastikan tidak kehilangan progress kerja secara tiba-tiba tanpa peringatan.
...
1. **Nama/Endpoint API** (method + path) — termasuk endpoint yang sudah disebut di prompt-prompt sebelumnya (`/monitoring/live`, endpoint reset tong) dan endpoint baru yang mungkin dibutuhkan khusus Petugas (contoh: endpoint untuk assignment area tugas, endpoint riwayat tong yang sudah dikosongkan oleh Petugas tertentu, endpoint notifikasi tong penuh).
...
6. **Auth/header & validasi role** — pastikan backend juga memvalidasi bahwa hanya role Petugas yang boleh mengakses endpoint reset/kosongkan tong (validasi role harus di backend, bukan cuma disembunyikan di UI mobile — ini penting dari sisi keamanan).
7. **Error case yang harus dihandle** (401, 403 — termasuk kasus role lain mencoba akses endpoint khusus Petugas, 404 tong tidak ditemukan, 409 tong sudah dikosongkan petugas lain di waktu bersamaan, 500, timeout/offline).
8. **File mobile yang terlibat**.

Endpoint yang **wajib dipastikan tersambung asli (bukan mock)**:
- Endpoint live monitoring tong (posisi, kapasitas, status).
- Endpoint reset/kosongkan tong.
- Endpoint notifikasi tong penuh (push ke Petugas).
...
- Pastikan modul Peta & Monitoring berfungsi penuh untuk Petugas, termasuk hak eksekusi "Kosongkan Tong" (berbeda dari Mahasiswa yang read-only, sesuai keputusan yang sudah dikonfirmasi sebelumnya).
...
1. **Kecepatan Akses Informasi Kritis**
   - Begitu buka aplikasi, Petugas harus langsung tahu: ada berapa tong penuh yang perlu segera ditangani, tanpa perlu banyak tap/scroll.
...
2. **Kejelasan Visual di Kondisi Lapangan**
   ...
   - Ukuran tombol "Kosongkan Tong" harus cukup besar dan mudah ditekan (Petugas mungkin memakai sarung tangan kerja atau kondisi terburu-buru).

3. **Alur Kosongkan Tong yang Cepat & Minim Kesalahan**
   - Dialog konfirmasi sebelum kosongkan tong harus singkat dan jelas (misalnya: "Kosongkan Tong [ID] di [alamat]? Aksi ini tidak bisa dibatalkan.") — hindari teks panjang yang bikin Petugas malas baca dan asal klik.
   - Setelah berhasil kosongkan tong, beri feedback jelas dan cepat (misalnya marker langsung berubah hijau + toast/snackbar konfirmasi singkat), supaya Petugas yakin aksinya berhasil tanpa harus refresh manual.

4. **Toleransi Koneksi Tidak Stabil**
   - Jika sinyal lemah saat menekan tombol kosongkan tong, beri indikator loading yang jelas dan retry otomatis/opsi retry manual, jangan sampai Petugas menekan berkali-kali karena tidak ada feedback (berisiko double action).
...
6. **Notifikasi yang Actionable**
   - Notifikasi tong penuh harus bisa langsung di-tap dan mengarahkan Petugas ke lokasi tong tersebut di peta, bukan hanya notifikasi pasif tanpa tindak lanjut.
