// Data ilustratif — bukan laporan aktual.

export const MAP_W = 1233;
export const MAP_H = 480;

// Proyeksi koordinat ilustratif → lat/lng di sekitar Kec. Coblong, Kota Bandung.
// Ganti `poly`/`pos` dengan koordinat GeoJSON nyata bila sudah tersedia.
const LAT0 = -6.883, LNG0 = 107.613, SPAN_KM = 6;
export const KM_PER_UNIT = SPAN_KM / MAP_W;
export function toLL([x, y]) {
  const dx = (x - MAP_W / 2) * KM_PER_UNIT, dy = (y - MAP_H / 2) * KM_PER_UNIT;
...
const P = (names) => names.split(" ").map((n) => V[n]);
...
KELURAHAN.forEach((k) => { k.ring = k.poly.map(toLL); k.labelLL = toLL(k.label); });
export const KEL_BY_ID = Object.fromEntries(KELURAHAN.map((k) => [k.id, k]));
...
export const TIPE_BY_ID = Object.fromEntries(TIPE.map((t) => [t.id, t]));
...
export const PLACES = [["Sukajadi", 95, 50], ["Cidadap", 1040, 58], ["Pasteur", 78, 250], ["Tamansari", 198, 392], ["Citarum", 620, 428], ["Cikutra", 1170, 356], ["← Bandung", 72, 366]]
  .map(([n, x, y]) => ({ nama: n, ll: toLL([x, y]) }));

// ---------- Kelas warna ----------
export const KEP_CLASSES = [
...
export const kepClass = (v) => KEP_CLASSES.find((c) => v >= c.min && v < c.max) || KEP_CLASSES[0];

export const CH4_CLASSES = [
...
export const ch4Class = (ppm) => CH4_CLASSES.find((c) => ppm >= c.min && ppm < c.max);
...
const insideWithMargin = (p, poly, m) =>
  inPoly(p, poly) && inPoly([p[0] + m, p[1]], poly) && inPoly([p[0] - m, p[1]], poly) &&
  inPoly([p[0], p[1] + m], poly) && inPoly([p[0], p[1] - m], poly);
...
function buildFacilities() {
  ...
  const bag = TIPE.flatMap((t) => Array(t.jumlah).fill(t.id));
  ...
  for (const kel of KELURAHAN) {
    const xs = kel.poly.map((p) => p[0]), ys = kel.poly.map((p) => p[1]);
    const [x0, x1, y0, y1] = [Math.min(...xs), Math.max(...xs), Math.min(...ys), Math.max(...ys)];
    for (let n = 0; n < kel.fac; n++) {
      let pt = null;
      const halfW = Math.max(58, kel.nama.length * 4.6 + 22); // zona label kelurahan
      for (let t = 0; t < 1200 && !pt; t++) {
        const c = [x0 + rand() * (x1 - x0), y0 + rand() * (y1 - y0)];
      ...
      if (!pt) { pt = [(x0 + x1) / 2 + n * 3, (y0 + y1) / 2 + n * 3]; FALLBACKS.push(kel.id); }
      const tipe = bag[idx];
      const rw = n < kel.rw ? n + 1 : 1 + Math.floor(rand() * kel.rw); // tiap RW minimal punya satu fasilitas
      out.push({
        id: `F${pad2(idx + 1)}`, tipe, kel: kel.id, rw, x: pt[0], y: pt[1],
...
export function statsAll(kelId, rw, pi) {
  ...
  const baseTotal = parts.reduce((a, p) => a + p.total / (MONTHLY_TOTAL[pi] / MONTHLY_TOTAL[CURRENT]), 0);
  return {
    org: sum("org"), ano: sum("ano"), res: sum("res"), total: sum("total"),
    kep: parts.length === 1 ? parts[0].kep : fac ? Math.round(kepW / fac) : 0, fac, sensors, baseTotal,
  };
}
