...
⚠️ **Penting:** ini ada **2 sistem polygon yang benar-benar terpisah**, jangan dicampur logic-nya.
...
- **Tidak ada CRUD sama sekali** — data ini murni log real (immutable, read-only untuk semua role, termasuk Super Admin tidak edit manual, hanya sistem yang menulis).
...
|------|----------------------------------------------------------|
| **Super Admin** | ✅ Akses penuh (satu-satunya yang boleh lihat data mentah lengkap) |
...
→ **Implikasi:** perlu dibuat 2 tampilan berbeda untuk data yang sama secara konsep — (1) tabel data mentah khusus Super Admin, dan (2) dashboard monitoring visual (chart/summary, bukan tabel mentah) khusus Admin Kelurahan/Kecamatan.
