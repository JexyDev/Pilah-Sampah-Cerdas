...
class EditProgramKerjaView extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<EditProgramKerjaView> createState() =>
      _EditProgramKerjaViewState();
}
...
class _EditProgramKerjaViewState extends ConsumerState<EditProgramKerjaView> {
  ...
  void _prefillForm(Map<String, dynamic> data) {
    _judulCtrl.text =
        data['judul']?.toString() ?? data['deskripsi']?.toString() ?? '';
    ...
    if (rawLower.contains('pemilahan') || rawLower.contains('pilah')) {
      _kategori = 'Pemilahan';
    } else if (rawLower.contains('pengangkutan') ||
        rawLower.contains('angkut')) {
      _kategori = 'Pengangkutan';
    } else if (rawLower.contains('pengolahan') || rawLower.contains('olah')) {
      _kategori = 'Pengolahan';
    } else if (rawLower.contains('pemanfaatan') ||
        rawLower.contains('manfaat')) {
      _kategori = 'Pemanfaatan';
    } else if (rawLower.contains('edukasi') ||
        rawLower.contains('sosialisasi')) {
    ...
    final waktu =
        data['waktuPelaksanaan']?.toString() ??
        data['targetTanggal']?.toString() ??
        '';
    if (waktu.contains(' s/d ')) {
      final parts = waktu.split(' s/d ');
      if (parts.length >= 2) {
        _tanggalMulaiCtrl.text = parts[0].trim();
        _tanggalSelesaiCtrl.text = parts[1].trim();
      }
  ...
  Future<void> _submit() async {
    ...
    if (_kategori == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Pilih kategori program kerja'),
    ...
    try {
      ...
      final payload = <String, dynamic>{
        ...
        'rencanaAnggaran': _anggaranCtrl.text.trim().isNotEmpty
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Gagal memperbarui program kerja.'),
            backgroundColor: AppColors.dangerRed,
          ),
        );
      }
  ...
  Widget _buildBanner() {
    ...
    if (status == 'PERLU_REVISI_DPL') {
      return _banner(
        ...
        title: '\u26a0\ufe0f Program Kerja Perlu Direvisi',
        body:
            'DPL meminta perubahan. Perbaiki sesuai catatan di bawah lalu simpan ulang.',
        catatan: catatanDpl,
        catatanLabel: 'Catatan Revisi dari DPL:',
      );
    }
    if (status == 'DITOLAK' || status == 'TIDAK_DISETUJUI') {
      return _banner(
        ...
        title: '\u274c Program Kerja Ditolak',
        body:
            'DPL menolak usulan program kerja ini. Perbaiki sesuai catatan lalu ajukan ulang.',
        catatan: catatanDpl,
    ...
    if (status == 'BELUM_DISETUJUI' || status.isEmpty) {
      return _banner(
        ...
        body:
            'Usulan masih menunggu persetujuan DPL. Anda dapat mengubah detail sebelum DPL merespons.',
        catatan: '',
        catatanLabel: '',
      );
    }
  ...
  Widget build(BuildContext context) {
    if (_isFetching) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Edit Program Kerja'),
          ...
          child: Column(
            ...
            children: [
              ...
              _buildField(
                ...
                hint: 'Judul singkat yang deskriptif',
                validator: (v) =>
                    v == null || v.isEmpty ? 'Masukkan judul' : null,
              ),
              const SizedBox(height: 14),
              _buildField(
                label: 'Deskripsi & Tujuan',
                controller: _deskripsiCtrl,
                hint: 'Jelaskan tujuan dan rencana kegiatan...',
                maxLines: 4,
              ...
              const SizedBox(height: 14),
              _buildField(
                label: 'Rencana Anggaran (Opsional)',
                ...
                children: [
                  Expanded(
                    child: _buildField(
                      ...
                      hint: 'YYYY-MM-DD',
                      readOnly: true,
                      onTap: () => _pickDate(_tanggalMulaiCtrl),
                    ),
                  ),
                  ...
                  Expanded(
                    child: _buildField(
                      ...
                      hint: 'YYYY-MM-DD',
                      readOnly: true,
                      onTap: () => _pickDate(_tanggalSelesaiCtrl),
                    ),
                  ),
                ],
              ),
              ...
              OutlinedButton.icon(
                ...
                label: Text(
                  _attachmentFile == null ? 'Pilih File' : 'Ganti File',
