...
export const dashboardService = {
  getKpi: async (wilayah?: string) => {
    ...
    const setoranHariIniKg = wasteLogsToday._sum.weightKg ? Number(wasteLogsToday._sum.weightKg) : 0;
    ...
    wasteByCategory.forEach(log => {
      ...
      if (log.category.name.toUpperCase().includes('ORGANIK') && !log.category.name.toUpperCase().includes('NON_ORGANIC') && !log.category.name.toUpperCase().includes('ANORGANIK')) {
        organikKg += kg;
      } else {
        anorganikKg += kg;
      }
