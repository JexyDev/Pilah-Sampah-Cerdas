...
const ManajemenTempatSampah: React.FC = () => {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `Laporan_Tempat_Sampah_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="text-sm">
            {loading ? (
              ...
              bins.map(bin => (
                <tr key={bin.kode} className="border-b border-outline-variant/30 hover:bg-surface-container-low/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex flex-col items-center gap-1 bg-white p-2 rounded-xl border border-outline-variant/60 w-fit shadow-sm">
                      ...
                      />
                      <span className="text-[10px] font-mono font-bold text-primary tracking-wider">{bin.kode}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button onClick={() => handleOpenEditModal(bin)} className="w-8 h-8 rounded-md bg-surface-container text-on-surface-variant hover:bg-primary hover:text-white transition-colors flex items-center justify-center" title="Edit">
                        <span className="material-symbols-outlined text-[18px]">edit</span>
                      </button>
                      <button onClick={() => handleDelete(bin.kode)} className="w-8 h-8 rounded-md bg-surface-container text-on-surface-variant hover:bg-red-500 hover:text-white transition-colors flex items-center justify-center" title="Hapus">
        ...
        <div className="fixed inset-0 bg-on-surface/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-2xl flex flex-col overflow-hidden">
            ...
            <div className="px-6 py-4 border-b border-outline-variant/30 flex justify-between items-center bg-surface-container-low">
              <div>
                <h3 className="text-[20px] font-bold text-on-surface">Log Transaksi Setoran Sampah</h3>
                <p className="text-[12px] font-bold text-on-surface-variant">Bin ID: {selectedBin}</p>
              </div>
              <button 
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh]">
              {loadingLogs ? (
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="text-[14px] text-on-surface">
                    {logTransactions.map((tx: any) => (
                      <tr key={tx.id} className="border-b border-outline-variant/30 hover:bg-surface-container-low transition-colors">
                        <td className="py-3 px-4">{new Date(tx.waktu).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}</td>
                        ...
                        <td className="py-3 px-4">
                          <span className={`font-bold ${tx.jenis.toUpperCase().includes('ORGANIK') ? 'text-primary' : 'text-secondary'}`}>
                            {tx.jenis}
                          </span>
                        </td>
                      </tr>
                ...
                <p className="text-center text-on-surface-variant text-sm py-8">Belum ada transaksi di tong ini</p>
              )}
            </div>
            {/* Modal Footer */}
      ...
      {isFormModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-outline-variant/30 flex justify-between items-center bg-surface-container-low">
              <h3 className="text-xl font-bold text-on-surface">{modalType === 'add' ? 'Tambah Titik Tempat Sampah' : 'Edit Tempat Sampah'}</h3>
              <button onClick={closeFormModal} className="text-on-surface-variant hover:bg-surface-container-low p-2 rounded-full transition-colors cursor-pointer">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4 overflow-y-auto max-h-[80vh]">
              <div>
                <label className="block text-sm font-medium text-on-surface mb-1">Kode / QR Code (opsional)</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-1">Kategori Sampah</label>
                <select 
                  required 
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-1">Wilayah RT/RW</label>
                <select 
                  required 
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-1">Kapasitas Maksimal (Liter)</label>
                <input 
                  type="number" 
                  required 
              ...
              <div className="mt-4 flex justify-end gap-3 pt-4 border-t border-outline-variant/30 bg-surface-container-low -mx-6 -mb-6 p-4">
                <button type="button" onClick={closeFormModal} className="px-4 py-2 rounded-lg font-medium text-on-surface-variant hover:bg-surface-container-low cursor-pointer">Batal</button>
                <button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-primary text-white rounded-lg font-medium hover:bg-primary/90 disabled:opacity-50 flex items-center gap-2 cursor-pointer">
