...
conn.on('ready', () => {
  ...
  const dumpCmd = `echo "${process.env.VPS_PASSWORD || process.env.VPS_PASS || 'Makerdotindo2026'}" | sudo -S docker exec psc-postgres pg_dump -U psc_user -d psc_db --clean --if-exists --no-owner --no-privileges`;
