...
|---|---|---|
| **Geofencing & Validasi Lokasi (Haversine)** | ✅ Safe & Enforced | Lokasi GPS Warga (`userLat`, `userLng`) divalidasi terhadap titik koordinat tong sampah fisik ($\le 10\text{m}$). Request manipulasi lokasi di luar radius ditolak server dengan error `LOCATION_OUT_OF_RANGE`. |
...
| **Realtime Sync Tanpa Refresh** | ✅ Clean & Fixed | Perubahan profil, poin, dan tong sampah langsung ter-update secara *live* tanpa perlu refresh manual atau login ulang. |
