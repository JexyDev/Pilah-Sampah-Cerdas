...
export const DEFAULT_TIMELINE_COBLONG = [
  ...
  },
  {
    ...
    fase: "Pra-Kegiatan",
    kegiatanUtama:
      "Koordinasi dengan Kepala Kelurahan & Dinas Lingkungan Hidup (DLH) untuk persiapan survey awal",
  ...
  },
  {
    ...
    fase: "Pra-Kegiatan",
    kegiatanUtama:
      "Survey lapangan di 6 kelurahan (Lebak Siliwangi, Lebak Gede, Cipaganti, Sadang Serang, Sekeloa, Dago)",
    outputTarget:
      "Data kondisi eksisting persampahan per kelurahan (titik TPS/TPS 3R, bank sampah, volume sampah) sebagai bahan baseline & lokasi posko",
  ...
  },
  {
    ...
    outputTarget:
      "DPL memahami mekanisme KKN, tata tertib, fokus program sampah, dan kebijakan/data teknis persampahan dari DLH",
    ...
    indikatorKeberhasilan: ["Format penilaian 50:50 dipahami oleh DPL dan tim mahasiswa"],
  },
  {
    tahapMinggu: "Pra-Kegiatan",
    ...
    outputTarget:
      "Mahasiswa memahami kode etik, tata tertib, kompetensi TIK & soft skills, dan profil wilayah Coblong",
    ...
    indikatorKeberhasilan: ["100% mahasiswa lulus pembekalan teknis dan siap diterjunkan"],
  },
  {
    tahapMinggu: "Pra-Kegiatan",
    ...
    tipsSukses: ["Lakukan check-in presensi pertama begitu tiba di radius posko/kelurahan."],
    checklist: [
  ...
  },
  {
    ...
    kegiatanUtama:
      "KICK OFF & Penerjunan: Pelepasan mahasiswa & keberangkatan DPL ke lokasi posko di 6 kelurahan; Koordinasi awal dengan RT/RW & verifikasi data baseline",
    outputTarget:
      "Posko terbentuk di 6 kelurahan; Mahasiswa & DPL siap di lokasi; Dokumen penerimaan resmi kelurahan",
  ...
  },
  {
    ...
    fase: "Fase 1 - Persiapan & Observasi",
    kegiatanUtama:
      "Observasi lapangan & pembuatan proposal/matrik program kerja; Identifikasi potensi-masalah sampah; Sosialisasi awal pemilahan & aplikasi BERSEKA",
    outputTarget:
      "Draft proposal & matrik program kerja tersusun; Warga RW binaan memahami pemilahan sampah",
  ...
  },
  {
    ...
    kegiatanUtama:
      "Finalisasi & persetujuan matrik program kerja oleh DPL & Kelurahan; Persiapan sarana pemilahan (tempat sampah 3 kategori, QR Code, materi edukasi); Pemilihan RT percontohan",
  ...
  },
  {
    ...
    outputTarget:
      "Bank sampah tercatat rapi; produk POC/buruan SAE mulai jalan; data komposisi sampah per wilayah",
  ...
  },
  {
    ...
    kegiatanUtama:
      "Konsolidasi capaian program seluruh kelurahan; Pengukuran indikator keberhasilan (perilaku, operasional, ekonomi sirkular, sistem digital); Finalisasi laporan akhir, video report, dan artikel ilmiah; Persiapan materi seminar hasil & publikasi konten digital; Upacara penarikan mahasiswa di Kantor Kecamatan Coblong; Unggah laporan akhir ke sistem informasi KKN",
    outputTarget:
      "Laporan akhir final, seminar akhir, Mahasiswa ditarik; laporan terunggah; nilai akhir keluar",
...
export const inferBidangKegiatan = (
  ...
  if (
    ...
    text.includes("door-to-door") ||
    text.includes("uji coba aplikasi")
  ) {
    return "Edukasi Warga & Sosialisasi";
  }
  if (
    text.includes("bank sampah") ||
    text.includes("maggot") ||
    text.includes("poc") ||
    text.includes("biopori") ||
    text.includes("kompos") ||
    text.includes("pengolahan")
  ) {
    return "Pengolahan & Bank Sampah";
  }
  if (
    text.includes("pengangkutan") ||
    text.includes("rute") ||
  ...
  if (
    text.includes("pemilahan") ||
    text.includes("sarana") ||
    text.includes("pilot") ||
    text.includes("tempat sampah") ||
    text.includes("observasi")
  ) {
...
export const resolveTimelineGuidance = (item: {
  ...
  const matched = DEFAULT_TIMELINE_COBLONG.find((d) => {
    ...
    if (
      minggu === dMinggu &&
      (keg.includes(dKeg.substring(0, 20)) || dKeg.includes(keg.substring(0, 20)))
    ) {
      return true;
    }
  ...
  if (fase.includes("pra") || minggu.includes("pra")) {
    ...
    checklist.push(
      "Koordinasi internal",
      "Kesiapan dokumen izin",
      "Perangkat & aplikasi mobile aktif"
    );
    indikatorKeberhasilan.push("Persiapan 100% lengkap sebelum hari penerjunan");
  } else if (
    fase.includes("observasi") ||
    minggu.includes("minggu 1") ||
    minggu.includes("minggu 2")
  ) {
    rekomendasiAksi.push(
    ...
    checklist.push(
      "Distribusi QR Code tempat sampah",
      "Aktivasi akun warga",
      "Uji coba setor sampah"
    );
    indikatorKeberhasilan.push("RT pilot aktif dengan kepatuhan pemilahan yang terverifikasi");
  } else if (
    fase.includes("implementasi") ||
    minggu.includes("minggu 5") ||
    minggu.includes("minggu 6") ||
    minggu.includes("minggu 7") ||
    minggu.includes("minggu 8") ||
    minggu.includes("minggu 9") ||
    minggu.includes("minggu 10") ||
    minggu.includes("minggu 11")
  ) {
    rekomendasiAksi.push(
    ...
    indikatorKeberhasilan.push(
      "Terjadi peningkatan volume sampah terpilah dan nasabah bank sampah"
    );
  } else {
    rekomendasiAksi.push(
    ...
    tipsSukses.push(
      "Pastikan semua bukti luaran dan logbook telah disetujui DPL sebelum penutupan."
    );
    checklist.push(
      "Laporan akhir KKN lengkap",
      "Video dokumenter & artikel",
      "Penilaian 50:50 DPL & Kelurahan",
      "Seminar hasil"
    );
...
export const timelineKknService = {
  ...
  getAll: async (params: TimelineQueryParams, userId?: string, userRole?: string) => {
    ...
    const resolvedItems = items.map((item) => {
      ...
      if (item.statusPelaksanaan !== dynamicStatus) {
        prisma.timelineKkn
          ...
          .catch((err) =>
            console.warn("[timelineKknService] auto-sync status warn:", err?.message)
          );
    ...
    let mapped = resolvedItems.map((item, idx) => {
      const bidang =
        (item as any).bidangKegiatan ||
        inferBidangKegiatan(item.kegiatanUtama, item.tahapMinggu, item.fase);
      const kelurahan =
        item.kelompok?.kelurahan || (item as any).kelurahan || "Coblong (Semua Wilayah)";
      ...
      return {
        ...
        urlGoogleDrive:
          (item as any).urlGoogleDrive ||
          (item as any).linkGoogleDrive ||
          "https://drive.google.com/drive/folders/kkn-coblong-2026",
        ...
        indikatorKeberhasilan:
          (item as any).indikatorKeberhasilan || guidance.indikatorKeberhasilan,
  ...
  getTimelineMahasiswa: async (params: TimelineQueryParams, userId?: string, userRole?: string) => {
    ...
    const totalSedangBerjalan = items.filter(
      (i) => i.statusPelaksanaan === "SEDANG_BERJALAN"
    ).length;
    const totalBelumDimulai = items.filter((i) => i.statusPelaksanaan === "BELUM_DIMULAI").length;

    const progressPercentage =
      totalTahapan > 0 ? Math.round((totalSelesai / totalTahapan) * 100) : 0;
    ...
    const faseMap = new Map<
      string,
      { fase: string; total: number; selesai: number; sedangBerjalan: number }
    >();
    items.forEach((item) => {
   ...
   */
  getActiveTimelineMahasiswa: async (
    ...
    try {
      ...
      const nextItem =
        activeIndex !== -1 && activeIndex + 1 < items.length ? items[activeIndex + 1] : null;
  ...
  updateStatus: async (id: string, statusPelaksanaan: string) => {
    ...
    if (!validStatuses.includes(statusPelaksanaan)) {
      throw new Error(
        "Status pelaksanaan tidak valid. Harus BELUM_DIMULAI, SEDANG_BERJALAN, atau SELESAI"
      );
    }
  ...
  bulkImport: async (
    ...
    const resolvedKelompokId =
      targetKelompokId && targetKelompokId !== "GLOBAL" ? targetKelompokId : null;
    ...
    for (const item of items) {
      ...
      const created = await prisma.timelineKkn.create({
        data: {
          ...
          kelompokId:
            item.kelompokId !== undefined
              ? item.kelompokId && item.kelompokId !== "GLOBAL"
                ? item.kelompokId
                : null
              : resolvedKelompokId,
        },
