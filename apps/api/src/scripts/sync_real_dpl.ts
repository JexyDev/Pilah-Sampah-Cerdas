...
async function syncDpl() {
  console.log("=== SINKRONISASI 32 DATA DPL & KELOMPOK STANDAR (NIP + PRODI) ===");
  ...
  for (const item of REAL_32_DPL_STANDARDIZED) {
    let existingUser = await prisma.user.findFirst({
      where: {
        OR: [
          ...
          { phone: item.rawPhone },
          {
            phone: item.rawPhone.startsWith("0")
              ? item.rawPhone.replace(/^0/, "+62")
              : `+62${item.rawPhone}`,
          },
    ...
    if (existingUser) {
      ...
      console.log(
        `[UPDATED USER #${item.no}] ${item.name} | NIP: ${item.nip} | Prodi: ${item.prodi}`
      );
    } else {
      try {
        ...
        console.log(
          `[CREATED USER #${item.no}] ${item.name} | NIP: ${item.nip} | Prodi: ${item.prodi}`
        );
      } catch (err) {
