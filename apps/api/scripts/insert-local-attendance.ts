...
async function main() {
  ...
  console.log("=== MEMULAI PENAMBAHAN PRESENSI & LOGBOOK DI LOCAL DB ===");

  // Ensure schedule exists in local DB
  const schedule = await prisma.schedule.upsert({
    where: { id: scheduleId },
    update: {
      title: "Kegiatan Harian Posko KKN Kelompok 4 Lebak Gede",
      date: new Date("2026-08-27T17:00:00.000Z"),
      time: "08:00 - 16:00",
      category: "POSKO_KKN",
      location: "Posko KKN Kelompok 4 Lebak Gede",
      latitude: lat,
      longitude: lng,
  ...
  for (const s of students) {
    ...
    console.log("  [OK] ActivityAttendance tersimpan.");
  ...
  console.log("\n=== SELESAI: LOCAL DB TELAH TERSINKRONISASI ===");
}
