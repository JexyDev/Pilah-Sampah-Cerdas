...
export const gamificationService = {
     ...
     */
    submitIdea: async (userId, judul, material, foto) => {
     ...
     */
    approveIdea: async (ideaId, adminUserId) => {
        return prisma.$transaction(async (tx) => {
            const idea = await tx.ideDaurUlang.findUnique({
                where: { id: ideaId },
            });
            if (!idea)
                throw new Error("RECYCLE_IDEA_NOT_FOUND");
    ...
    getLeaderboard: async () => {
        ...
        const citizenLeaderboard = individualUsers
            .map((u) => {
            const totalPoints = u.pointHistory.reduce((acc, cur) => acc + cur.points, 0);
            return {
                id: u.id,
                name: u.name,
        ...
        const pengangkutLeaderboard = petugasUsers
            .map((p) => {
            const completedTasks = p.claimedTasks.filter((t) => t.status === "COMPLETED");
    ...
    getLeaderboardKkn: async () => {
        ...
        const studentLeaderboard = students.map((s) => {
            let totalHours = 0;
            ...
            const activeBinsCount = s.user.registeredBins.filter((b) => b.status === "ACTIVE_BOUND").length;
        ...
        const dplLeaderboard = dplUsers
            ...
            d.dplKelompok.forEach((kel) => {
                kel.students.forEach((s) => {
                    totalStudentCount++;
                    let totalHours = 0;
                    s.user.attendances.forEach((att) => {
                        if (att.checkOutAt && att.attendedAt) {
                    ...
                    const activeBins = s.user.registeredBins.filter((b) => b.status === "ACTIVE_BOUND").length;
