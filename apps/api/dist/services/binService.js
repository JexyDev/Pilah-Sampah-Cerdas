...
export class BinService {
                    ...
                    if (!existingTask) {
                        ...
                        if (targetBin.latitude && targetBin.longitude) {
                            ...
                            for (const p of petugas) {
                                ...
                                if (lat && lng) {
                                    const dist = getDistanceInMeters({ lat: Number(targetBin.latitude), lng: Number(targetBin.longitude) }, { lat: Number(lat), lng: Number(lng) });
                                    if (dist < minDist) {
                                        minDist = dist;
                                        assignedUserId = p.id;
                                    }
    ...
    async createResetRequest(binId, userId, evidencePhotoUrl) {
        ...
        const isOwner = bin.userId === userId || bin.binOwnerships.some((o) => o.userId === userId);
        if (!isOwner) {
            throw new Error("BIN_NOT_OWNED");
        }
     ...
     */
    async listResetRequests(currentUser, filters) {
        let whereClause = {};
        if (currentUser) {
            const { getScopingFilters } = await import("../utils/rbacScoping.js");
            const scoping = await getScopingFilters(currentUser);
