...
export const dashboardService = {
    getKpi: async (wilayah, period, startDate, endDate) => {
        ...
        const getRtRwIdMatch = (wilayahStr) => {
            if (areaIds.length > 0)
                return { rwId: { in: areaIds } };
            return { rw: { name: { contains: wilayahStr, mode: "insensitive" } } };
        };
        ...
        const hhWhere = {};
        if (isFiltered)
            hhWhere.rw = getRtRwMatch(wilayah);
        if (dateFilter)
            hhWhere.createdAt = dateFilter;
        ...
        if (isFiltered)
            wasteLogsWhere.warga = { rw: getRtRwMatch(wilayah) };
        if (dateFilter)
        ...
        const binsWhere = {};
        if (isFiltered)
            binsWhere.rw = getRtRwMatch(wilayah);
        if (dateFilter)
        ...
        if (isFiltered)
            setoranHariIniWhere.warga = { rw: getRtRwMatch(wilayah) };
        if (dateFilter)
        ...
        const catWhere = {};
        if (isFiltered)
            catWhere.warga = { rw: getRtRwMatch(wilayah) };
        if (dateFilter)
        ...
        residuLogs.forEach((log) => {
            residuKg += Number(log.berat);
        });
        // 10. Jadwal Tugas
        const jadwalWhere = {};
        if (dateFilter)
            jadwalWhere.createdAt = dateFilter;
    ...
    getRecentTransactions: async (wilayah) => {
        const isFiltered = wilayah &&
            wilayah !== "Kecamatan Coblong" &&
            wilayah !== "Sistem Pusat" &&
            wilayah !== "Area KKN Dago" &&
    ...
    getTrend: async (weeks = 8, wilayah) => {
        const isFiltered = wilayah &&
            wilayah !== "Kecamatan Coblong" &&
            wilayah !== "Sistem Pusat" &&
            wilayah !== "Area KKN Dago" &&
        ...
        for (let i = effectiveWeeks - 1; i >= 0; i--) {
            ...
            const numberOfDays = Math.floor((endOfWeek.getTime() - oneJan.getTime()) / (24 * 60 * 60 * 1000));
