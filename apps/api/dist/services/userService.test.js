...
describe("UserService", () => {
    ...
    describe("createUser", () => {
        it("should create a user successfully when inputs are valid", async () => {
            ...
            vi.mocked(userRepository.findRoleByName).mockResolvedValue(mockRole);
        ...
        it("should throw EMAIL_CONFLICT if email already exists", async () => {
            vi.mocked(userRepository.findRoleByName).mockResolvedValue({ id: 1 });
            vi.mocked(userRepository.findByEmail).mockResolvedValue({ id: "exist-user" });
            await expect(userService.createUser({
                name: "New Warga",
