...
vi.mock("@prisma/client", () => {
    ...
    return {
        PrismaClient: class {
            ...
            setoranManual = mPrisma.setoranManual;
...
vi.mock("./configService.js", () => {
    return {
        configService: {
            getConfig: vi.fn().mockImplementation(async (key) => {
...
describe("Batch 2 Core Features", () => {
    ...
    describe("binService - claimDispatchTask", () => {
        ...
        it("should throw error if task is already claimed", async () => {
            ...
            await expect(binService.claimDispatchTask("task-1", "petugas-2")).rejects.toThrow("DISPATCH_TASK_ALREADY_CLAIMED");
        });
    });
    describe("aiService - discrepancy check", () => {
        it("should set discrepancyStatus to PENDING_REVIEW if classification mismatches and confidence > 90%", async () => {
            ...
            mockUpdate.mockImplementation(async (args) => args.data);
            const res = await aiService.submitPetugasReport("log-1", "petugas-1", 2.5, "NON_ORGANIC", "GPS");
