...
export class KknAttendanceService {
    async pingLocation(userId, latitude, longitude) {
    ...
    async getWargaDampingan(userId) {
     ...
     */
    async updateStudentLocationsBatch(studentId, locations) {
        ...
        if (!latestLoc)
            return { locations: [], autoAttendanceTriggered: [] };
        ...
        for (const schedule of activeSchedules) {
            ...
            if (schedule.polygon && Array.isArray(schedule.polygon) && schedule.polygon.length >= 3) {
                const polyPoints = schedule.polygon.map((p) => ({
                    lat: Number(p[0]),
                    lng: Number(p[1]),
                }));
     ...
     */
    async getActivityLocation(scheduleId) {
    ...
    async getActiveStudentsLocations() {
        ...
        for (const mhs of allMahasiswa) {
            if (!uniqueStudents.has(mhs.id)) {
                ...
                else if (mhs.studentProfile?.assignedRw?.latitude &&
     ...
     */
    async getAttendanceList(scheduleId) {
        ...
        const attendedList = list.map((att) => {
            ...
            if (latestLoc) {
                let isInside = false;
                if (scheduleLoc.polygon &&
                    Array.isArray(scheduleLoc.polygon) &&
                    scheduleLoc.polygon.length >= 3) {
                    const polyPoints = scheduleLoc.polygon.map((p) => ({
                        lat: Number(p[0]),
                        lng: Number(p[1]),
                    }));
                    isInside = isPointInPolygon({ lat: Number(latestLoc.latitude), lng: Number(latestLoc.longitude) }, polyPoints);
                }
                else {
                    const dist = calculateDistance(Number(latestLoc.latitude), Number(latestLoc.longitude), scheduleLoc.latitude, scheduleLoc.longitude);
        ...
        let studentWhereCondition = { role: { name: "MAHASISWA_KKN" } };
        if (schedule?.kelompok?.students && schedule.kelompok.students.length > 0) {
            ...
            const matchedGroup = sortedGroups.find((g) => schedule.title.toLowerCase().includes(g.name.toLowerCase()));
