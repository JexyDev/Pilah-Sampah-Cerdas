...
describe("AuthService - registerWarga security", () => {
    beforeEach(() => {
        ...
        vi.mocked(authRepository.findRoleByName).mockResolvedValue({ id: 1, name: "WARGA" });
    });
    it("should throw PIC_MISMATCH when Mahasiswa KKN registers a bin assigned to another PIC", async () => {
        ...
        vi.mocked(binRepository.findByQrCode).mockResolvedValue(mockBin);
        vi.mocked(binRepository.findQrBatchById).mockResolvedValue(mockBatch);
        await expect(authService.registerWarga({ phone: "081234567890", password: "password123", name: "Warga" }, {}, qrCode, wargaSubtype, scannerUser)).rejects.toThrow("PIC_MISMATCH");
    });
    it("should allow registration when Mahasiswa KKN registers a bin assigned to themselves", async () => {
        ...
        vi.mocked(binRepository.findByQrCode).mockResolvedValue(mockBin);
        vi.mocked(binRepository.findQrBatchById).mockResolvedValue(mockBatch);
        vi.mocked(authRepository.registerWargaTx).mockResolvedValue({ id: "warga-1" });
        const result = await authService.registerWarga({ phone: "081234567890", password: "password123", name: "Warga" }, {}, qrCode, wargaSubtype, scannerUser);
