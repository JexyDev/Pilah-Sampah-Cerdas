...
function getKelompokWhere(dplUserId, role) {
    const normalizedRole = String(role || "").toUpperCase();
    const isAdmin = ["ADMIN_DLH", "DLH", "DLH_ADMIN", "superUser", "SUPER_USER", "ADMIN"].some((r) => normalizedRole.includes(r));
    if (isAdmin) {
        return {};
    }
    return {
...
export const dplService = {
     ...
     */
    getGroupSummary: async (dplUserId, role) => {
        ...
        const groupSummaries = await Promise.all(groups.map(async (grp, idx) => {
            ...
            const avgAttendanceRate = expectedAttendances > 0 && totalAttendances > 0
     ...
     */
    getStudentDetails: async (dplUserId, groupId, role) => {
        const whereGroup = getKelompokWhere(dplUserId, role);
        if (groupId)
        ...
        const studentDetails = await Promise.all(students.map(async (st) => {
     ...
     */
    getAssistedCitizens: async (dplUserId, studentId) => {
        ...
        const citizenList = await Promise.all(bins.map(async (bin) => {
     ...
     */
    getMapCoverage: async (dplUserId, role) => {
        let groups = await prisma.kelompokKkn.findMany({
            where: getKelompokWhere(dplUserId, role),
            select: {
                id: true,
     ...
     */
    getAlerts: async (dplUserId, role) => {
        let groups = await prisma.kelompokKkn.findMany({
            where: getKelompokWhere(dplUserId, role),
     ...
     */
    getApprovalHistory: async (dplUserId, role) => {
        const normalizedRole = String(role || "").toUpperCase();
        const isAdmin = ["ADMIN_DLH", "DLH", "DLH_ADMIN", "superUser", "SUPER_USER", "ADMIN"].some((r) => normalizedRole.includes(r));
     ...
     */
    assessStudent: async (dplUserId, studentId, score, note) => {
        const student = await prisma.studentKkn.findFirst({
            where: {
                OR: [{ id: studentId }, { userId: studentId }],
            },
     ...
     */
    decideLeaveRequest: async (dplUserId, requestId, status, rejectionReason) => {
