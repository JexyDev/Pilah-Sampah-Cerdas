...
export class ResiduService {
    async recordViolation(petugasUserId, data) {
        ...
        let multiplier = 1;
        if (data.severity === "MEDIUM" || data.severity === "SEDANG")
            multiplier = 2;
        if (data.severity === "SEVERE" || data.severity === "TINGGI")
            multiplier = 3;
        ...
        if (citizen.phone) {
            await notificationIntegrationService.sendWhatsApp(citizen.phone, `Halo ${citizen.name}, petugas residu mendeteksi residu tercampur di tempat sampah Anda (${data.type}). Mohon pastikan memilah sampah organik dan anorganik dengan benar. Poin Anda berkurang -${pointsToDeduct}.`);
        }
        return {
            id: result.id,
    ...
    async getDashboardSummary(petugasUserId, _period = "hari") {
        ...
        const zoneLabel = petugas.assignedZone ||
    ...
    async getRiwayat(petugasUserId, _range, type) {
        const logs = [];
        ...
        if (!type || type === "SEMUA" || type === "SETORAN") {
            ...
            logs.push(...setoranOtomatis.map((s) => {
    ...
    async getAnalytics() {
        ...
        const zones = await Promise.all(allRw.map(async (rw) => {
    ...
    async submitLog(petugasUserId, data) {
        ...
        const lokasiGps = data.latitude && data.longitude ? `${data.latitude}, ${data.longitude}` : null;
        const pointRateConfig = await configService.getConfig("point_rate_per_kg");
        const pointRatePerKg = pointRateConfig ? parseInt(pointRateConfig, 10) : 2;
