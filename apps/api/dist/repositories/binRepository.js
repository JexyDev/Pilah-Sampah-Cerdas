...
export class BinRepository {
     ...
     */
    async recordScanTransaction(householdId, binId, categoryId, weightKg, volumeLiter, requestId, userId, pointsAwarded, categoryName, aiConfidence, evidencePhotoUrl) {
    ...
    async updateResetRequestStatus(id, status, reviewedById) {
     ...
     */
    async createQrBatch(batchCode, quantity) {
        return prisma.$transaction(async (tx) => {
     ...
     */
    async assignQrBatch(batchId, assignedPicUserId, adminUserId) {
        return prisma.$transaction(async (tx) => {
