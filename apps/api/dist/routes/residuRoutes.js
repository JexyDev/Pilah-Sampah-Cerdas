...
const verifiedPetugasGuard = async (req, res, next) => {
    if (req.user?.role === "PETUGAS_RESIDU") {
        ...
        const isApproved = user.status === "Aktif" ||
            user.status === "ACTIVE" ||
 ...
 */
router.get("/pending-logs", authMiddleware, roleMiddleware(["PETUGAS_RESIDU"]), verifiedPetugasGuard, residuController.getPendingLogs);
/**
 * @swagger
 ...
 */
router.get("/jadwal-harian", authMiddleware, roleMiddleware(["PETUGAS_RESIDU"]), verifiedPetugasGuard, residuController.getJadwalHarian);
/**
 * @swagger
 ...
 */
router.post("/violation", authMiddleware, roleMiddleware(["PETUGAS_RESIDU"]), verifiedPetugasGuard, uploadResiduImage, residuController.recordViolation);
/**
 * @swagger
 ...
 */
router.post("/submit-log", authMiddleware, roleMiddleware(["PETUGAS_RESIDU"]), verifiedPetugasGuard, uploadResiduImage, residuController.submitLog);
/**
 * @swagger
 ...
 */
router.get("/dashboard", authMiddleware, roleMiddleware(["PETUGAS_RESIDU"]), verifiedPetugasGuard, residuController.getDashboardSummary);
/**
 * @swagger
 ...
 */
router.get("/analytics", authMiddleware, roleMiddleware(["PETUGAS_RESIDU"]), verifiedPetugasGuard, residuController.getAnalytics);
/**
 * @swagger
 ...
 */
router.get("/riwayat", authMiddleware, roleMiddleware(["PETUGAS_RESIDU"]), verifiedPetugasGuard, residuController.getRiwayat);
