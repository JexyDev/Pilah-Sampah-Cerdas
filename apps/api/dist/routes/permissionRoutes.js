...
router.get("/me", async (req, res) => {
    try {
        ...
        const result = Object.fromEntries(permissions.map((p) => [
            p.resource,
            {
                canView: p.canView,
...
router.get("/", async (req, res) => {
    try {
        ...
        const data = roles.map((role) => ({
            roleId: role.id,
            roleName: role.name,
            permissions: Object.fromEntries(role.permissions.map((p) => [
                p.resource,
                {
                    canView: p.canView,
                    canCreate: p.canCreate,
...
router.put("/:roleId", async (req, res) => {
    try {
        ...
        const ops = Object.entries(permissions).map(([resource, perms]) => prisma.permission.upsert({
