...
export class BinController {
    ...
    async registerWargaBin(req, res) {
        try {
            const data = req.body;
    ...
    async getNextQr(req, res) {
        try {
            ...
            const nextQr = await generateNextQrCode(categoryId);
    ...
    async listResetRequests(req, res) {
        try {
            ...
            const result = await binService.listResetRequests(currentUser, status ? { status: String(status) } : undefined);
            res.status(200).json({ success: true, data: result });
        }
        catch (error) {
     ...
     */
    async reviewResetRequest(req, res) {
        try {
     ...
     */
    async approveResetRequest(req, res) {
        try {
            const reviewedById = req.user.userId;
            const { id } = req.params;
    ...
    async approveActivation(req, res) {
        try {
            const { id } = req.params;
            const adminUserId = req.user.userId;
    ...
    async createResetRequestMobile(req, res) {
        try {
            const userId = req.user.userId;
