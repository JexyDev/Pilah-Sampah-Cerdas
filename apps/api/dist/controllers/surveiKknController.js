...
export class SurveiKknController {
     ...
     */
    async getImportHistory(_req, res) {
        try {
     ...
     */
    async downloadTemplate(_req, res) {
        try {
    ...
    async getAllSurveys(req, res) {
        try {
            ...
            const result = await surveiKknService.getAllSurveys(page, limit, search);
            res.status(200).json({
                success: true,
                data: result.data,
                meta: result.meta,
