...
export const transactionController = {
    ...
    createManualDeposit: async (req, res) => {
        try {
            ...
            const result = await transactionService.createManualDeposit(finalPetugasId, finalDiinputOleh, parseFloat(berat), photoPath, lokasiGps || null, rwId ? parseInt(rwId, 10) : undefined);
            res.status(201).json({
                success: true,
    ...
    getManualDeposits: async (req, res) => {
        try {
            const user = req.user;
            let rwId;
