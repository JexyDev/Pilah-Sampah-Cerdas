          ...
          <div className="space-y-6">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      <tr>
                        <td className="py-3.5 text-slate-600 font-medium flex items-center gap-2"><MapPin size={16} /> Kecamatan</td>
                        <td className="py-3.5 text-right font-bold text-slate-800">{renderValue(selectedSurvey.kecamatan)}</td>
                      </tr>
                      <tr>
                        <td className="py-3.5 text-slate-600 font-medium flex items-center gap-2"><Map size={16} /> Jumlah RW</td>
                        <td className="py-3.5 text-right font-bold text-slate-800">{renderValue(selectedSurvey.jumlahRw, "RW")}</td>
                      </tr>
                      <tr>
                        <td className="py-3.5 text-slate-600 font-medium flex items-center gap-2"><Map size={16} /> Jumlah RT</td>
                        <td className="py-3.5 text-right font-bold text-slate-800">{renderValue(selectedSurvey.jumlahRt, "RT")}</td>
                      </tr>
                      <tr>
                        <td className="py-3.5 text-slate-600 font-medium flex items-center gap-2"><Users size={16} /> Jumlah KK</td>
                        <td className="py-3.5 text-right font-bold text-slate-800">{renderValue(selectedSurvey.jumlahKk, "KK")}</td>
                      </tr>
                      <tr>
                        <td className="py-3.5 text-slate-600 font-medium flex items-center gap-2"><Home size={16} /> Total Rumah</td>
                        <td className="py-3.5 text-right font-bold text-slate-800">{renderValue(selectedSurvey.jumlahRumahTotal)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-y-6 gap-x-4 text-sm flex-1 content-start">
                  ...
                  <div>
                    ...
                    <div className="font-bold text-slate-800">{renderValue(selectedSurvey.enumerator)}</div>
                  </div>
                  <div>
                    <span className="text-slate-500 font-medium block mb-1">Titik Kumpul</span>
                    <div className="font-bold text-slate-800">{renderValue(selectedSurvey.titikKumpulMahasiswa)}</div>
                  </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 text-center flex flex-col items-center justify-center min-h-[140px]">
                  <p className="text-xs text-slate-700 font-bold uppercase tracking-wider mb-2">Total</p>
                  {vs?.totalVolumeKgPerHari !== undefined && vs?.totalVolumeKgPerHari !== null ? (
                    <>
                      <p className="text-5xl font-black text-slate-800 mb-1">{vs.totalVolumeKgPerHari}</p>
              ...
              {vs?.catatan && <p className="text-sm text-slate-500 mt-4 bg-slate-50 p-4 rounded-xl border border-slate-100 italic">Catatan: {vs.catatan}</p>}
            </div>
          </div>
        );
      case "karakter":
          ...
          <div className="space-y-6 max-w-5xl mx-auto">
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    <tr>
                      <td className="py-3.5 text-slate-600 font-medium">Padat Penduduk</td>
                      <td className="py-3.5 text-right">{renderBool(kw.padatPenduduk)}</td>
                    </tr>
                    <tr>
                      <td className="py-3.5 text-slate-600 font-medium">Banyak Kos/Kontrakan</td>
                      <td className="py-3.5 text-right">{renderBool(kw.banyakKosKontrakan)}</td>
                    </tr>
                    <tr>
                      <td className="py-3.5 text-slate-600 font-medium">Banyak UMKM/Warung</td>
                      <td className="py-3.5 text-right">{renderBool(kw.banyakUmkmWarungKafe)}</td>
                    </tr>
                    <tr>
                      <td className="py-3.5 text-slate-600 font-medium">Dekat Kampus/Sekolah</td>
                      <td className="py-3.5 text-right">{renderBool(kw.dekatKampusSekolah)}</td>
                    </tr>
                    <tr>
                      <td className="py-3.5 text-slate-600 font-medium">Dekat Pasar</td>
                      <td className="py-3.5 text-right">{renderBool(kw.pasar)}</td>
                    </tr>
                    <tr>
                      <td className="py-3.5 text-slate-600 font-medium">Dekat Bantaran Sungai</td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-center">
                ...
                <p className="text-2xl font-black text-slate-800">{renderValue(kw.perkiraanJumlahKosKontrakan)}</p>
              </div>
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-center">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Perkiraan Jml UMKM</p>
                <p className="text-2xl font-black text-slate-800">{renderValue(kw.perkiraanJumlahUmkmWarungKafe)}</p>
              </div>
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-center md:col-span-1">
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10 mb-8">
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 flex flex-col justify-center">
                  ...
                  <p className="text-4xl font-black text-slate-800">{renderValue(ps.jumlahRumahMemilah)}</p>
                </div>
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 flex flex-col justify-center">
                  <p className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2">Total Rumah (Sample)</p>
                  <p className="text-4xl font-black text-slate-800">{renderValue(ps.totalJumlahRumahDiRw)}</p>
                </div>
              </div>
              ...
              <div className="bg-emerald-50 border border-emerald-100 p-8 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                ...
                <div className="text-left md:text-right">
                  ...
                  <span className="inline-block bg-white text-emerald-700 px-5 py-2 rounded-xl font-bold border border-emerald-200 text-lg shadow-sm">
          ...
          <div className="space-y-6 max-w-5xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-emerald-50/50 p-6 rounded-2xl border border-emerald-100 shadow-sm flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-4xl font-black text-slate-800">{renderValue(bs.bankSampahAktif)}</p>
                </div>
              </div>
              <div className="bg-rose-50/50 p-6 rounded-2xl border border-rose-100 shadow-sm flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-4xl font-black text-slate-800">{renderValue(bs.bankSampahTidakAktif)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-2 text-sm mb-8">
                ...
                <div className="flex justify-between items-center py-3 border-b border-slate-50">
                  <span className="text-slate-600 font-medium">Ecobrick / Daur Ulang</span> {renderBool(bs.ecobrickKerajinanDaurUlang)}
                </div>
                <div className="flex justify-between items-center py-3 border-b border-slate-50">
                  <span className="text-slate-600 font-medium">Buruan Sae / Kebun Warga</span> {renderBool(bs.buruanSae)}
                </div>
                <div className="flex justify-between items-center py-3 border-b border-slate-50">
                  <span className="text-slate-600 font-medium">Pengepul Mitra</span> {renderBool(bs.pengepulMitraDaurUlang)}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  ...
                  <p className="text-3xl font-black text-slate-800">{renderValue(bs.jumlahUnitKomposter)}</p>
                </div>
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Titik Maggot / BSF</p>
                  <p className="text-3xl font-black text-slate-800">{renderValue(bs.jumlahTitikMaggotBsf)}</p>
                </div>
                ...
                <tbody className="divide-y divide-slate-100 text-sm">
                  {kp.map((actor: any, idx: number) => (
                    <tr key={actor.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 font-extrabold text-slate-800">{renderValue(actor.nama)}</td>
                      ...
                      <td className="px-6 py-4 text-slate-600 font-medium">{renderValue(actor.kontak)}</td>
                      <td className="px-6 py-4 text-slate-600">{renderValue(actor.peran)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-300 pb-20">
                    ...
                    {completion.breakdown.map((b, i) => (
                      <div key={i} className="flex items-center gap-1.5">
                        {b.isComplete ? (
                          ...
                          <div className="w-3 h-3 rounded-full bg-slate-200 shrink-0 border border-slate-300" />
                        )}
      ...
      <div className="min-h-[500px]">
        {isLoadingDetail ? (
          <div className="flex flex-col items-center justify-center py-32 bg-slate-50/50 rounded-2xl border border-slate-100">
