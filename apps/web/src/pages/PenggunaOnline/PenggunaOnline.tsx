...
export const PenggunaOnline: React.FC = () => {
  ...
  const getRoleBadgeStyle = (role: string) => {
    ...
    if (r.includes("super"))
      return "bg-purple-50 text-purple-700 border-purple-200/90 font-bold";
    if (r.includes("lurah") || r.includes("camat") || r.includes("admin"))
      return "bg-sky-50 text-sky-700 border-sky-200/90 font-bold";
    if (r.includes("dpl") || r.includes("mahasiswa"))
      return "bg-indigo-50 text-indigo-700 border-indigo-200/90 font-bold";
    ...
    if (r.includes("rw") || r.includes("rt"))
      return "bg-teal-50 text-teal-700 border-teal-200/90 font-bold";
    return "bg-emerald-50 text-[#009966] border-emerald-200/90 font-bold";
  };
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs">
        <div className="flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-2xl bg-[#e5f7ed] text-[#009966] flex items-center justify-center border border-[#009966]/20 shadow-2xs shrink-0">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black text-[#009966]">
                {onlineUsers.length}
              </span>
              <span className="text-xs font-semibold text-slate-400">Pengguna</span>
            </div>
            <span className="text-[11px] text-[#009966] font-medium block">
              • Token refresh aktif &lt; 7 hari
            </span>
          </div>
          <div className="w-11 h-11 rounded-2xl bg-[#e5f7ed] text-[#009966] flex items-center justify-center border border-[#009966]/20 shrink-0">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="flex items-baseline gap-1.5">
              ...
              <span className="text-xs font-semibold text-slate-400">Perangkat</span>
            </div>
            <span className="text-[11px] text-indigo-600 font-medium block">
              • Android / iOS Mobile App
            </span>
          </div>
          <div className="w-11 h-11 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center border border-indigo-200 shrink-0">
            <Smartphone size={22} />
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-emerald-100 text-[#009966] border border-emerald-300">
          ...
          <div className="flex items-center gap-3 shrink-0 flex-wrap sm:flex-nowrap">
            ...
            <div className="relative w-full sm:w-56 md:w-64">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-xs font-semibold bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#009966] focus:bg-white transition-all"
              />
            </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td
                    ...
                    className="py-14 text-center text-slate-400 font-medium"
                  >
                    <Globe size={34} className="mx-auto mb-2 opacity-30 text-slate-400" />
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8.5 h-8.5 rounded-xl bg-[#e5f7ed] text-[#009966] font-black text-xs flex items-center justify-center border border-emerald-200 shrink-0">
                            {userItem.name.charAt(0).toUpperCase()}
                          </div>
                          <div>
                            <span className="font-extrabold text-slate-900 group-hover:text-[#009966] transition-colors block text-xs">
                              {userItem.name}
                            </span>
                      ...
                      <td className="py-3.5 px-4">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-bold bg-slate-100 text-slate-700 border border-slate-200">
                          {isMob ? (
                            ...
                            <Monitor size={12} className="text-sky-500 shrink-0" />
                          )}
                          {userItem.device}
                        </span>
                      </td>
                      ...
                      {isPrivilegedUser && (
                        <td className="py-3.5 px-4 text-center">
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white transition-all inline-flex items-center justify-center gap-1 font-extrabold text-[10px] cursor-pointer border border-rose-200 hover:shadow-xs active:scale-95"
        ...
        <div className="p-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs font-semibold text-slate-500 bg-slate-50/50">
          <div>
            Menampilkan{" "}
            ...
            dari <span className="text-slate-900 font-bold">{filteredUsers.length}</span> sesi aktif
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="px-2 py-1 bg-white border border-slate-200 rounded-lg text-slate-700 font-bold focus:outline-none cursor-pointer"
              >
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="px-3 py-1 bg-slate-100 rounded-lg font-extrabold text-slate-700 hover:bg-slate-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer transition-colors"
              >
                &lt;
              </button>
          ...
          >
            ...
            <div className="p-6 space-y-4">
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2.5">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-extrabold text-slate-900 text-sm">{targetUser.name}</h4>
                      <div className="flex items-center gap-2 text-xs text-slate-500 font-mono">
                        <span>{targetUser.phone}</span>
                        <span>•</span>
                        <span className="text-rose-600 font-bold">{targetUser.identifier}</span>
                      </div>
                    </div>
                  </div>
                  <span className="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase bg-rose-50 text-rose-700 border border-rose-200">
                    {targetUser.role}
                  </span>
                </div>
              </div>
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none focus:border-rose-500 focus:bg-white transition-colors cursor-pointer"
                >
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 text-xs font-medium bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:border-rose-500 focus:bg-white transition-colors"
                />
              </div>
              <div className="p-3.5 rounded-xl bg-rose-50 border border-rose-200 flex items-start gap-2.5 text-xs text-rose-800">
                ...
                <p className="leading-relaxed">
                  <strong className="text-rose-900">Peringatan:</strong> Seluruh Refresh Token
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-3">
              <button
                ...
                className="px-4 py-2.5 bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 font-extrabold text-xs rounded-xl transition-colors cursor-pointer"
              >
                Batal
              </button>
              <button
