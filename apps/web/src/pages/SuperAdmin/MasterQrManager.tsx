...
export const MasterQrManager: React.FC = () => {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post("/super-admin/bins/generate-qr", {
        totalQr,
        categoryId,
  ...
  const handleReactivate = async (binId: string) => {
    try {
      const res = await api.put(`/super-admin/bins/${binId}/reactivate`, {});
      if (res.data?.success) {
  ...
  const handleUpdateStatus = async (binId: string, newStatus: string) => {
    try {
      const res = await api.put(`/super-admin/bins/${binId}/status`, { status: newStatus });
      if (res.data?.success) {
  ...
  const handleExecuteReplace = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post(`/super-admin/bins/${selectedOldBin.id}/replace`, {
        newBinId: newQrInput,
      });
      if (res.data?.success) {
  ...
  const handleDeleteBin = async (binId: string, qrCode: string) => {
    ...
    try {
      const res = await api.delete(`/super-admin/bins/${binId}`);
      if (res.data?.success) {
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
          ...
          <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-wrap gap-3 justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <option value="">Semua Status Bin</option>
