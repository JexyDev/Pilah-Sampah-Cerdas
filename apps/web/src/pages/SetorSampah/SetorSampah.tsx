  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 text-slate-800 dark:text-slate-100 font-sans">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4">
        {/* Total Weight Card */}
        <div className="bg-white dark:bg-slate-900 p-4.5 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-2xs flex items-center gap-3.5 group hover:border-emerald-300 dark:hover:border-emerald-700 transition-all">
          ...
          <div>
            <p className="text-[10.5px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-wider">Total Sampah Terpilah</p>
            <p className="text-lg font-black text-slate-900 dark:text-slate-100 mt-0.5">
              ...
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400">{totalBerat >= 1000 ? "Ton" : "Kg"}</span>
            </p>
          </div>
        </div>

        {/* Total Points Card */}
        <div className="bg-white dark:bg-slate-900 p-4.5 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-2xs flex items-center gap-3.5 group hover:border-amber-300 dark:hover:border-amber-700 transition-all">
          ...
          <div>
            <p className="text-[10.5px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-wider">Poin Diterbitkan</p>
            <p className="text-lg font-black text-amber-700 dark:text-amber-400 mt-0.5">{totalPoinRounded.toLocaleString("id-ID")} <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Pts</span></p>
          </div>
        </div>

        {/* AI Confidence Card */}
        <div className="bg-white dark:bg-slate-900 p-4.5 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-2xs flex items-center gap-3.5 group hover:border-purple-300 dark:hover:border-purple-700 transition-all">
          ...
          <div>
            <p className="text-[10.5px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-wider">Akurasi Model AI</p>
            <p className="text-lg font-black text-purple-700 dark:text-purple-400 mt-0.5">{akurasiAi}% <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">(Presisi)</span></p>
          </div>
        </div>

        {/* Total Verified Setoran Card */}
        <div className="bg-white dark:bg-slate-900 p-4.5 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-2xs flex items-center gap-3.5 group hover:border-blue-300 dark:hover:border-blue-700 transition-all">
          ...
          <div>
            <p className="text-[10.5px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-wider">Setoran Terverifikasi</p>
            <p className="text-lg font-black text-blue-700 dark:text-blue-400 mt-0.5">{totalItems} <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Transaksi</span></p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className={`px-3.5 py-2.5 border rounded-2xl text-xs font-bold transition ${
              isLurah
                ? "bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 cursor-not-allowed opacity-90"
                : "bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 outline-none focus:border-[#009966] cursor-pointer"
            }`}
          >
          ...
          <select
            ...
            className="px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-2xl text-xs font-bold outline-none focus:border-[#009966] transition cursor-pointer"
          >
      ...
      <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-2xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100 dark:border-slate-800">
          <div>
            <h3 className="font-extrabold text-base text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-2">
              <Calendar size={18} className="text-[#009966] dark:text-emerald-400" /> Log Aktivitas Penyetoran Sampah Real-Time
            </h3>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 text-slate-400 dark:text-slate-500 border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl space-y-3">
            <ScanLine size={36} className="mx-auto text-slate-300 dark:text-slate-600" />
            ...
            {(searchQuery || filterKelurahan !== "ALL" || filterRw !== "ALL" || filterCategory !== "ALL" || filterPeriode !== "ALL") && (
              <button
                onClick={resetFilters}
                className="px-4 py-2 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 rounded-xl text-xs font-bold border border-emerald-200 dark:border-emerald-700/50 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 transition inline-flex items-center gap-1.5 cursor-pointer"
              >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-center align-middle">
                        <span className="font-extrabold text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-lg border border-slate-200 dark:border-slate-700">
                          {confidenceVal}%
                        </span>
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-center align-middle">
                        {realPhoto ? (
                          <button
                            ...
                            className="group relative inline-flex items-center gap-1.5 p-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl border border-slate-200 dark:border-slate-700 transition-all cursor-pointer"
                            title="Klik untuk memperbesar foto asli upload mobile"
                          >
                            <div className="w-8 h-8 rounded-lg overflow-hidden border border-slate-300 dark:border-slate-600 bg-slate-200 dark:bg-slate-700 shrink-0">
                              <img
                                src={realPhoto}
                            ...
                            <span className="text-[10px] font-extrabold text-slate-700 dark:text-slate-300 pr-1 group-hover:text-[#009966] dark:group-hover:text-emerald-400">
                              Foto Upload
                            </span>
                          </button>
                        ) : (
                          <span className="inline-block text-[10px] font-bold text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-xl border border-slate-200 dark:border-slate-700">
                            Tanpa Foto
                          </span>
                        )}
                      </td>
      ...
      {selectedLogForDetail && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 max-w-xl w-full overflow-hidden">
            {/* Modal Header */}
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-gradient-to-r from-emerald-50/80 dark:from-emerald-950/40 to-white dark:to-slate-900">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] font-semibold text-slate-400 dark:text-slate-400">
                    ID Transaksi: <span className="font-mono text-emerald-700 dark:text-emerald-400">{selectedLogForDetail.id}</span>
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div className="flex items-center gap-3 p-3.5 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700">
                ...
                <div className="min-w-0 flex-1">
                  <h4 className="font-extrabold text-slate-900 dark:text-slate-100 text-xs sm:text-sm truncate">{cleanWargaName(selectedLogForDetail.warga)}</h4>
                  <div className="flex flex-wrap items-center gap-2 mt-1">
                    <span className="inline-block bg-[#eef5ff] dark:bg-blue-950/60 text-[#2b6cb0] dark:text-blue-300 font-bold text-[11px] px-2.5 py-0.5 rounded-lg border border-[#c3dafe] dark:border-blue-700/50">
                      {formatRukunWarga(selectedLogForDetail.rw || selectedLogForDetail.rtRw)}
                    </span>
                    <span className="inline-block bg-[#e8f8f0] dark:bg-emerald-950/60 text-[#009966] dark:text-emerald-400 font-bold text-[11px] px-2.5 py-0.5 rounded-lg border border-[#b8ebd0] dark:border-emerald-700/50">
                    ...
                    {selectedLogForDetail.phone && (
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 font-semibold flex items-center gap-1">
                        <Phone size={11} className="text-[#009966] dark:text-emerald-400" /> {selectedLogForDetail.phone}
                      </span>
                    )}
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-[11px] font-bold text-slate-400 dark:text-slate-500 pt-1">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-2xl border border-slate-200/80 dark:border-slate-700 space-y-1">
                  <span className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500">Berat Timbangan</span>
                  <p className="font-mono font-black text-[#009966] dark:text-emerald-400 text-sm">{selectedLogForDetail.berat} Kg</p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-2xl border border-slate-200/80 dark:border-slate-700 space-y-1">
                  <span className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500">Poin Terdistribusi</span>
                  <p className="font-mono font-black text-amber-600 dark:text-amber-400 text-sm">+{Math.round(selectedLogForDetail.poin)} Pts</p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-2xl border border-slate-200/80 dark:border-slate-700 space-y-1">
                  <span className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500">Status Audit</span>
                ...
                <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-2xl border border-slate-200/80 dark:border-slate-700 space-y-1">
                  <span className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500">Waktu Pencatatan</span>
              ...
              <div className="p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/50 text-emerald-900 dark:text-emerald-300 text-xs font-semibold flex items-center gap-2">
            ...
            <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/80 flex justify-end">
              <button
                onClick={() => setSelectedLogForDetail(null)}
                className="px-5 py-2.5 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-xl text-xs font-black hover:bg-slate-800 dark:hover:bg-white transition cursor-pointer"
              >
                Tutup Detail
              </button>
            </div>
      ...
      {previewImageUrl && (
        ...
        >
          <div className="relative max-w-3xl w-full max-h-[90vh] flex items-center justify-center">
            ...
            <button
              onClick={() => setPreviewImageUrl(null)}
              className="absolute -top-4 -right-4 w-10 h-10 rounded-full bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-bold flex items-center justify-center shadow-xl cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700"
            >
              <X size={20} />
            </button>
          </div>
