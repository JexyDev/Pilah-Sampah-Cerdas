  ...
  return (
    <div className="max-w-7xl mx-auto py-6 px-4 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="py-12 text-center text-slate-400 font-bold text-xs">
        ...
        {/* Table Footer Pagination matching Manajemen Tempat Sampah (Gambar Referensi) */}
        <div className="px-5 py-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-50/50">
          ...
          <div className="flex items-center gap-4">
            <div className="text-xs text-slate-500 font-medium">
              Menampilkan <span className="font-bold text-slate-800">{filteredProvinsi.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}–{Math.min(currentPage * itemsPerPage, filteredProvinsi.length)}</span> dari <span className="font-bold text-[#009966]">{filteredProvinsi.length} data</span>
            </div>
            <Pagination
