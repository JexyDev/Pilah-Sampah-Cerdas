...
const GoogleDriveIcon = () => (
  <svg className="w-3.5 h-3.5 shrink-0" viewBox="0 0 87.3 78" fill="none">
    ...
    <path
      d="M59.8 53H87.3c0-1.55-.4-3.1-1.2-4.5l-19.9-34.5c-.8-1.4-1.95-2.5-3.3-3.3z"
      fill="#2684fc"
    />
    <path
      d="m73.55 76.8-13.75-23.8H27.5l-13.75 23.8c1.35.8 2.9 1.2 4.5 1.2h50.8c1.6 0 3.15-.4 4.5-1.2z"
      fill="#ffba00"
    />
  </svg>
);
...
export const ProgramKerjaKkn: React.FC = () => {
  ...
  const formatWaktuPelaksanaanDisplay = (rawStr?: string | null) => {
    ...
    if (parts.length === 2) {
      ...
      if (!isNaN(d1.getTime()) && !isNaN(d2.getTime())) {
        const f1 = d1.toLocaleDateString("id-ID", {
          day: "numeric",
          month: "long",
          year: "numeric",
        });
        const f2 = d2.toLocaleDateString("id-ID", {
          day: "numeric",
          month: "long",
          year: "numeric",
        });
  ...
  const handleRejectSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dplService.decideProgramKerja(rejectModal.id, "DITOLAK", rejectModal.catatanDpl.trim());
  ...
  const normalizeStatusUsulan = (
    statusUsulan?: string,
    legacyStatus?: string
  ): "BELUM_DISETUJUI" | "DISETUJUI" | "DITOLAK" | "KADALUARSA" => {
  ...
  const normalizeStatusPelaksanaan = (
    ...
    if (p === "SEDANG_BERJALAN" || p === "SEDANG_DILAKSANAKAN" || p === "BERJALAN")
  ...
  const handleExportXlsx = () => {
    ...
    toast.success(
      `Data program kerja (${filteredProkers.length} baris) berhasil diekspor ke XLSX!`
    );
  };
      ...
      case "DITOLAK":
        return (
          <div className="flex flex-col items-center gap-1" title={catatanDpl || "Ditolak DPL"}>
            ...
            {catatanDpl && (
              <span
                className="text-[10px] text-rose-600 dark:text-rose-400 font-semibold max-w-[150px] truncate"
                title={catatanDpl}
              >
                {catatanDpl}
              </span>
      ...
      case "KADALUARSA":
        return (
          <div
            ...
            title={catatanDpl || "Dibatalkan otomatis oleh sistem (H+5)"}
          >
            ...
            {catatanDpl && (
              <span
                className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold max-w-[150px] truncate"
                title={catatanDpl}
              >
                {catatanDpl}
              </span>
    ...
    <div className="space-y-6 text-slate-800 dark:text-slate-100">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1">
            Menampilkan status usulan (persetujuan DPL) dan status pelaksanaan program kerja
            mahasiswa KKN.
          </p>
        </div>
              ...
              <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300 border-collapse">
                <thead>
                  <tr className="bg-slate-50/90 dark:bg-slate-800/90 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 text-[11px] uppercase tracking-wider font-bold">
                    ...
                    {isDeveloper && (
                      <th className="py-3.5 px-3 min-w-[190px] text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-medium">
                  {paginatedProkers.map((p, idx) => {
                    ...
                    return (
                      <tr
                        key={p.id}
                        className="hover:bg-slate-50/80 dark:bg-slate-800/80 dark:hover:bg-slate-800/50 transition-colors"
                      >
                                ...
                                <div>
                                  ...
                                  {p.penginput.nim && (
                                    <p className="text-[10px] text-slate-400 font-mono mt-0.5">
                                      NIM: {p.penginput.nim}{" "}
                                      {p.penginput.prodi ? `• ${p.penginput.prodi}` : ""}
                                    </p>
                                  )}
                                </div>
                              ) : (
                        ...
                        <td className="py-3.5 px-3 text-center">{renderSumberBadge(p.sumber)}</td>
                        <td className="py-3.5 px-4">
                          <p
                            className="text-slate-900 dark:text-slate-100 font-bold line-clamp-2"
                            title={p.judul || "-"}
                          >
                            {p.judul || "-"}
                          </p>
                        ...
                        <td className="py-3.5 px-4 max-w-xs">
                          <p
                            className="text-slate-900 dark:text-slate-100 leading-relaxed font-normal line-clamp-3"
                            title={p.deskripsi}
                          >
                            {p.deskripsi}
                          </p>
                          ...
                          <td className="py-3.5 px-4 text-center">
                            <div className="flex items-center justify-center gap-1.5 flex-nowrap">
                              <button
                                ...
                                className="p-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors cursor-pointer shrink-0"
                              >
                                <Pencil size={13} />
                              </button>
                              <button
                                ...
                                className="p-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/50 transition-colors cursor-pointer shrink-0"
                              >
                                <Trash2 size={13} />
                              </button>
                            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      <p
                        className="text-slate-600 dark:text-slate-300 text-xs leading-relaxed line-clamp-3"
                        title={p.deskripsi}
                      >
                        {p.deskripsi}
                      </p>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs pt-1">
                      <div className="p-2.5 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800">
                        ...
                        <span
                          className="font-semibold text-slate-800 dark:text-slate-200 text-xs mt-0.5 block truncate"
                          title={p.waktuPelaksanaan || "-"}
                        >
                          {formatWaktuPelaksanaanDisplay(p.waktuPelaksanaan)}
                        </span>
                      </div>
      ...
      {isFormModalOpen && (
        ...
        >
          <div
            ...
            className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-4xl shadow-2xl border border-slate-100 dark:border-slate-800 space-y-4 max-h-[90vh] overflow-y-auto"
          >
                  ...
                  <div>
                    ...
                    {isDpl && kelompokList.length <= 1 ? (
                      <div className="w-full px-3.5 py-2.5 bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-800 dark:text-slate-100 font-bold flex items-center justify-between">
                        <span>
                          {kelompokList[0]?.name || "Kelompok Binaan"} (
                          {kelompokList[0]?.kelurahan
                            ? `Kel. ${kelompokList[0]?.kelurahan}`
                            : "Wilayah Binaan"}
                          )
                        </span>
                ...
                <div className="space-y-4">
                  <div className="flex flex-col h-[142px]">
                    ...
                    <textarea
                      ...
                      className="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 outline-none flex-grow resize-none"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, kebutuhanBiaya: Number(e.target.value) })
                      }
                    ...
                    <>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setFormData({ ...formData, statusUsulan: e.target.value as any })
                            }
                            className="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-800 dark:text-slate-100 font-semibold focus:ring-2 focus:ring-emerald-500 cursor-pointer"
                          >
                            <option value="BELUM_DISETUJUI">Menunggu Persetujuan</option>
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setFormData({ ...formData, statusPelaksanaan: e.target.value as any })
                            }
                            className="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-800 dark:text-slate-100 font-semibold focus:ring-2 focus:ring-emerald-500 cursor-pointer"
                          >
                            <option value="BELUM_MULAI">Belum Mulai</option>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          className="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 outline-none resize-none"
                        />
                      </div>
                    </>
                  )}
      ...
      {rejectModal.isOpen && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-md shadow-2xl border border-slate-100 dark:border-slate-800 space-y-4"
          >
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
              Anda akan menolak rencana kegiatan:{" "}
              <span className="font-bold text-slate-800 dark:text-slate-200">
                "{rejectModal.deskripsi}"
              </span>
              . Silakan berikan catatan revisi atau alasan penolakan bagi mahasiswa.
            </p>
      ...
      {rosterModal.isOpen && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-2xl shadow-2xl border border-slate-100 dark:border-slate-800 space-y-4 max-h-[85vh] flex flex-col"
          >
            {/* Modal Header */}
            <div className="flex items-start justify-between pb-3 border-b border-slate-100 dark:border-slate-800 shrink-0">
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-3 text-[11px] text-slate-500 dark:text-slate-400 pt-1 flex-wrap">
                  ...
                  {rosterModal.cakupanRw &&
                    Array.isArray(rosterModal.cakupanRw) &&
                    rosterModal.cakupanRw.length > 0 && (
      ...
      {detailModal.isOpen && detailModal.proker && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-2xl shadow-2xl border border-slate-100 dark:border-slate-800 space-y-4 max-h-[90vh] overflow-y-auto"
          >
            {/* Header */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200/60 dark:border-slate-800">
                ...
                <div>
                  {renderStatusUsulanBadge(
                    detailModal.proker.statusUsulan,
                    detailModal.proker.status,
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200/60 dark:border-slate-800">
                ...
                <div>
                  {renderStatusPelaksanaanBadge(
                    detailModal.proker.statusPelaksanaan,
                    detailModal.proker.status
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1 text-xs">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                    {detailModal.proker.kelurahan
                      ? `Kelurahan ${detailModal.proker.kelurahan}`
                      : ""}
                    {detailModal.proker.cakupanRw &&
                    Array.isArray(detailModal.proker.cakupanRw) &&
                    detailModal.proker.cakupanRw.length > 0
                  ...
                  {detailModal.proker.penginput && (
                    <div className="mt-1 pt-1 border-t border-emerald-200/40 dark:border-emerald-800/40">
                      ...
                      <span className="font-semibold text-slate-800 dark:text-slate-200">
                        {detailModal.proker.penginput.nama}{" "}
                        {detailModal.proker.penginput.nim
                          ? `(${detailModal.proker.penginput.nim})`
                          : ""}
                      </span>
                    </div>
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2">
                {canModifyProker &&
                  (() => {
                    const p = detailModal.proker;
                    const normalizedU = normalizeStatusUsulan(p.statusUsulan, p.status);
                    const normalizedP = p.statusPelaksanaan || "BELUM_MULAI";
                    return (
                      <>
                        {normalizedU === "BELUM_DISETUJUI" && (
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-rose-50 dark:bg-rose-950/50 hover:bg-rose-100 text-rose-600 border border-rose-200 font-bold text-xs transition-all cursor-pointer"
                            >
                        ...
                        {(normalizedU === "DITOLAK" || normalizedU === "KADALUARSA") && (
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 font-bold text-xs transition-all cursor-pointer"
                          >
                            <Check size={13} strokeWidth={3} />
                            <span>Setujui</span>
                          </button>
                          ...
                          <div className="flex items-center gap-1.5">
                            {normalizedP === "BELUM_MULAI" && (
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs transition-all cursor-pointer"
                              >
                            ...
                            {normalizedP !== "SELESAI" && (
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-rose-50 text-slate-600 hover:text-rose-600 border border-slate-200 font-bold text-xs transition-all cursor-pointer"
                              >
                                <X size={13} />
                                <span>Tolak</span>
                              </button>
                ...
                <button
                  type="button"
                  onClick={() => setDetailModal({ isOpen: false, proker: null })}
                  className="px-4 py-1.5 text-xs font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition-colors cursor-pointer ml-1"
                >
                  Tutup
                </button>
              </div>
