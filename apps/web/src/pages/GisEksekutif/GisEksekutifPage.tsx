...
export default function GisEksekutifPage() {
  ...
  const prev = useMemo(
    () => (pi > 0 ? statsAll(kelId, rwN, pi - 1) : null),
    [kelId, rwN, pi]
  );
  ...
  const kelRows = useMemo<ComplianceRow[]>(
    () =>
      KELURAHAN.map((k) => ({
        k,
        s: statsKel(k, kelId === k.id ? rwN : null, pi),
      })),
    [kelId, rwN, pi]
  );
  ...
  const series = useMemo(
    () =>
      MONTHLY_TOTAL.map(
        (v) => (v / MONTHLY_TOTAL[CURRENT]) * stats.baseTotal
      ),
    [stats.baseTotal]
  );
  ...
  const scoped = useMemo(
    () =>
      FASILITAS.filter(
        (f) => (!kelId || f.kel === kelId) && (!rwN || f.rw === rwN)
      ),
    [kelId, rwN]
  );
  ...
  const visible = useMemo<FacilityItem[]>(() => {
    ...
    return scoped.filter((f) => {
      ...
      if (!q) return true;
      const kelNama = KEL_BY_ID[f.kel]?.nama || "";
      const tipeFungsi = TIPE_BY_ID[f.tipe]?.fungsi || "";
      const hay = `${f.nama} ${kelNama} rw ${f.rw} rw ${pad2(f.rw)} ${tipeFungsi}`.toLowerCase();
      return q.split(/\s+/).every((tok) => hay.includes(tok));
    });
  }, [scoped, facType, query]);

  const filtered = facType !== "all" || query.trim() !== "";
  const sensorScope = SENSORS;
  ...
  const toggleKel = (id: string) => changeKel(kel === id ? "all" : id);
    ...
    if (what === "kel") {
      ...
      KELURAHAN.forEach((k) => {
        ...
        rows.push([
          ...
          dec(s.total),
          s.kep,
          `${live.length}/${s.sensors.length}`,
          live.length ? Math.max(...live.map((z) => z.ppm as number)) : "",
        ]);
      });
      ...
      visible.forEach((f) =>
        rows.push([
          f.id,
          f.nama,
          TIPE_BY_ID[f.tipe]?.nama || f.tipe,
          TIPE_BY_ID[f.tipe]?.fungsi || "-",
          KEL_BY_ID[f.kel]?.nama || f.kel,
          pad2(f.rw),
        ])
      );
  ...
  const scopeText = !kelId
    ...
    : `di Kel. ${KEL_BY_ID[kelId]?.nama || kelId}${rwN ? ` RW ${pad2(rwN)}` : ""}`;
  const totalRounded =
    stats.total >= 100
      ? Math.round(stats.total)
      : Math.round(stats.total * 10) / 10;
        ...
        <section className="kpis" aria-label="Indikator utama">
          ...
          <div className="card kpi">
            ...
            <div>
              ...
              <div className="kpi-v">
                {fmtN(totalRounded)} <small>m³/bulan</small>
              </div>
              <div
                className={`kpi-s ${
                  volDelta === null ? "" : volDelta >= 0 ? "up" : "down"
                }`}
              >
                {volDelta === null ? (
                  noPrev
          ...
          <div className="card kpi">
            ...
            <div>
              ...
              <div
                className={`kpi-s ${
                  kepDelta === null ? "" : kepDelta >= 0 ? "up" : "down"
                }`}
              >
                {kepDelta === null ? (
                  noPrev
                  ...
                  <>
                    {kepDelta >= 0 ? "▲" : "▼"} {kepDelta >= 0 ? "+" : "−"}
                    {Math.abs(kepDelta)} poin{" "}
                    <span className="mut">dari periode sebelumnya</span>
                  </>
                )}
              </div>
        ...
        <section className="main">
          ...
          <aside className="card side" aria-label="Legenda">
            ...
            <p className="side-note">
              Lokasi, batas, angka, dan grafik bersifat ilustratif; bukan laporan aktual.
            </p>
          </aside>
        </section>
