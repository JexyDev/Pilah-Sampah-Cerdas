    ...
    <section className="py-16 md:py-24 bg-white border-b border-slate-200/80 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 bg-slate-50/70 p-6 md:p-8 rounded-3xl border border-slate-200/70 shadow-sm space-y-6">
            <h3 className="text-sm font-bold text-slate-800 text-center uppercase tracking-wider mb-6">
                ...
                return (
                  <div key={item.id} className="space-y-1">
                    <div className="flex justify-between items-center text-xs font-bold text-slate-700">
                      <span>{item.name}</span>
                      <span className="text-slate-900">{item.mhsCount} Mahasiswa</span>
                    </div>
                    <div className="h-9 w-full bg-slate-200/80 rounded-xl overflow-hidden p-1 flex items-center">
                      <div
                        className="h-full bg-[#1D3B2F] rounded-lg transition-all duration-700 ease-out flex items-center justify-end px-3"
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="bg-slate-50/70 rounded-3xl border border-slate-200/70 shadow-sm overflow-hidden">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-200/80 text-xs font-medium text-slate-800">
                  {KKN_DISTRIBUTION_DATA.map((item: KelurahanDistribution, idx: number) => (
                    <tr
                      key={item.id}
                      className={idx % 2 === 1 ? "bg-slate-100/50 hover:bg-slate-100" : "bg-white hover:bg-slate-50"}
                    >
                      <td className="py-3 px-4 font-bold text-slate-900">{item.name}</td>
                      <td className="py-3 px-3 text-center font-bold">{item.mhsCount}</td>
                      <td className="py-3 px-3 text-center font-bold text-[#1D3B2F]">{item.klpCount}</td>
                      <td className="py-3 px-3 text-center font-bold text-[#1D3B2F]">{item.dplCount}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="bg-slate-200/70 text-xs font-extrabold text-slate-900 border-t-2 border-slate-300">
                    ...
                    <td className="py-3.5 px-3 text-center text-[#1D3B2F]">{KKN_DISTRIBUTION_TOTALS.totalKlp}</td>
                    <td className="py-3.5 px-3 text-center text-[#1D3B2F]">{KKN_DISTRIBUTION_TOTALS.totalDpl}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
