...
/// ─────────────────────────────────────────────────────────────────────────────
/// Model untuk Kegiatan KKN Aktif & Background GPS Tracking (v2)
/// ─────────────────────────────────────────────────────────────────────────────
class KegiatanKknLokasi extends Equatable {
  final String alamat;
  final double latitude;
  final double longitude;
  ...
  factory KegiatanKknLokasi.fromJson(Map<String, dynamic> json) {
    List<List<double>>? parsedPolygon;
    if (json['polygon'] != null && json['polygon'] is List) {
      try {
        final list = json['polygon'] as List;
        parsedPolygon = list.map<List<double>>((pt) {
          final pts = pt as List;
          return [
    ...
    return KegiatanKknLokasi(
      alamat: json['alamat']?.toString() ?? json['location']?.toString() ?? 'Lokasi Kegiatan KKN',
      ...
      radiusMeter: double.tryParse(json['radiusMeter']?.toString() ?? json['radius']?.toString() ?? '150') ?? 150.0,
  ...
  @override
  List<Object?> get props => [alamat, latitude, longitude, radiusMeter, polygon];
}

class KegiatanKknKelompok extends Equatable {
  final String id;
  final String nama;

  const KegiatanKknKelompok({
    required this.id,
    required this.nama,
  });

  factory KegiatanKknKelompok.fromJson(Map<String, dynamic> json) {
    return KegiatanKknKelompok(
      id: json['id']?.toString() ?? '',
      nama: json['nama']?.toString() ?? json['name']?.toString() ?? 'Kelompok KKN',
    );
  }
...
class KegiatanKknItem extends Equatable {
  final String id;
  final String namaKegiatan;
  final String tanggal;
  final String jamMulai;
  final String jamSelesai;
  final int durasiWajibMenit;
  final KegiatanKknLokasi lokasi;
  final String status; // 'AKTIF' | 'BELUM_MULAI' | 'SELESAI' | 'LIBUR'
  final String? statusKehadiran; // null | 'HADIR' | 'IZIN' | 'SAKIT' | 'ALPA' | 'BERLANGSUNG'
  final KegiatanKknKelompok kelompok;
  ...
  const KegiatanKknItem({
    ...
    required this.durasiWajibMenit,
  ...
  bool get isAktif => status.toUpperCase() == 'AKTIF';
  bool get isBelumMulai => status.toUpperCase() == 'BELUM_MULAI';
  bool get isSelesai => status.toUpperCase() == 'SELESAI';
  bool get isLibur => status.toUpperCase() == 'LIBUR';

  bool get isHadir => statusKehadiran?.toUpperCase() == 'HADIR';
  bool get isIzin => statusKehadiran?.toUpperCase() == 'IZIN';
  bool get isSakit => statusKehadiran?.toUpperCase() == 'SAKIT';
  bool get isAlpa => statusKehadiran?.toUpperCase() == 'ALPA';
  bool get isBerlangsung => statusKehadiran?.toUpperCase() == 'BERLANGSUNG';
  bool get isBelumPresensi => statusKehadiran == null || isBerlangsung;

  factory KegiatanKknItem.fromJson(Map<String, dynamic> json) {
    return KegiatanKknItem(
      id: json['id']?.toString() ?? '',
      namaKegiatan: json['namaKegiatan']?.toString() ?? json['title']?.toString() ?? 'Kegiatan KKN',
      tanggal: json['tanggal']?.toString() ?? '',
      jamMulai: json['jamMulai']?.toString() ?? '08:00',
      jamSelesai: json['jamSelesai']?.toString() ?? '16:00',
      durasiWajibMenit: (json['durasiWajibMenit'] as num?)?.toInt() ?? 120,
      lokasi: json['lokasi'] != null && json['lokasi'] is Map<String, dynamic>
          ? KegiatanKknLokasi.fromJson(json['lokasi'] as Map<String, dynamic>)
          : KegiatanKknLokasi.fromJson(json),
      status: json['status']?.toString().toUpperCase() ?? 'AKTIF',
      statusKehadiran: json['statusKehadiran']?.toString().toUpperCase(),
      kelompok: json['kelompok'] != null && json['kelompok'] is Map<String, dynamic>
          ? KegiatanKknKelompok.fromJson(json['kelompok'] as Map<String, dynamic>)
          : const KegiatanKknKelompok(id: 'KLP-001', nama: 'Kelompok KKN'),
    );
  }
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'durasiWajibMenit': durasiWajibMenit,
      'lokasi': lokasi.toJson(),
      'status': status,
      'statusKehadiran': statusKehadiran,
      'kelompok': kelompok.toJson(),
    };
  }

  KegiatanKknItem copyWith({
    String? id,
    String? namaKegiatan,
    String? tanggal,
    String? jamMulai,
    String? jamSelesai,
    int? durasiWajibMenit,
    KegiatanKknLokasi? lokasi,
    String? status,
    String? statusKehadiran,
    KegiatanKknKelompok? kelompok,
  }) {
    return KegiatanKknItem(
      id: id ?? this.id,
      ...
      durasiWajibMenit: durasiWajibMenit ?? this.durasiWajibMenit,
...
class MulaiKegiatanResponse extends Equatable {
  ...
  factory MulaiKegiatanResponse.fromJson(Map<String, dynamic> json) {
    return MulaiKegiatanResponse(
      sessionId: json['sessionId']?.toString() ?? '',
      scheduleId: json['scheduleId']?.toString() ?? json['id']?.toString() ?? '',
      namaKegiatan: json['namaKegiatan']?.toString() ?? '',
      jamMulai: json['jamMulai']?.toString() ?? '08:00',
      jamSelesai: json['jamSelesai']?.toString() ?? '16:00',
      durasiWajibMenit: (json['durasiWajibMenit'] as num?)?.toInt() ?? 120,
      lokasi: json['lokasi'] != null && json['lokasi'] is Map<String, dynamic>
          ? KegiatanKknLokasi.fromJson(json['lokasi'] as Map<String, dynamic>)
          : KegiatanKknLokasi.fromJson(json),
      serverTimestamp: json['serverTimestamp']?.toString() ?? DateTime.now().toIso8601String(),
      attendanceId: json['attendanceId']?.toString(),
    );
  }

  @override
  List<Object?> get props => [sessionId, scheduleId, attendanceId];
}

class OutOfZoneViolationResult extends Equatable {
  final bool success;
  final String message;
  final int penaltyPoints;
  final int? remainingPoints;

  const OutOfZoneViolationResult({
  ...
  factory OutOfZoneViolationResult.fromJson(Map<String, dynamic> json) {
    final data = json['data'] as Map<String, dynamic>? ?? {};
    return OutOfZoneViolationResult(
      success: json['success'] == true,
      message: json['message']?.toString() ?? 'Pelanggaran keluar zona tercatat.',
      penaltyPoints: (data['penaltyPoints'] as num?)?.toInt() ?? (data['pointsDeducted'] as num?)?.toInt() ?? 10,
      remainingPoints: (data['remainingPoints'] as num?)?.toInt(),
    );
  }

  @override
  List<Object?> get props => [success, message, penaltyPoints, remainingPoints];
}
