...
class ApiBinRepository implements BinRepository {
  ...
  Future<AiDetectionEntity> detectWaste(String userId) async {
    try {
      final response = await apiClient.dio.post('/ai/predict', data: {
        'userId': userId,
      });

      if (response.statusCode == 200) {
        final data = response.data['data'];
        final String jenisSampah = data['jenis_sampah'];
        final double volume = (data['estimasi_volume'] as num).toDouble();
        return AiDetectionEntity(
          detectedType: jenisSampah == 'ORGANIC' ? WasteType.organic : WasteType.nonOrganic,
          volumeEstimate: volume,
          isBlurry: false,
        );
      }
      throw const BinException('AI_ERROR', 'Gagal mendeteksi sampah');
    } catch (e) {
      throw const BinException('NETWORK_ERROR', 'Gagal memproses deteksi AI');
    }
  }
