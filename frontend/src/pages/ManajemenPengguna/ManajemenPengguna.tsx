...
const ManajemenPengguna: React.FC = () => {
  ...
  const uniqueRws = Array.from(new Set(areas.map(a => {
    const match = a.name.match(/RW\s+(\d+)/i);
    return match ? match[1] : null;
  }).filter(Boolean))).sort() as string[];

  const uniqueRts = Array.from(new Set(areas.map(a => {
  ...
  const handleOpenAddModal = () => {
    setModalType('add');
    setFormData({ name: '', email: '', password: '', roleName: 'WARGA', nik: '', status: 'Aktif', rtRwId: areas[0]?.id?.toString() || '' });
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Nama Lengkap', 'Email', 'NIK', 'Peran', 'Wilayah', 'Setoran (kg)', 'Status', 'Tanggal Terdaftar'];
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm">
              {loading ? (
                ...
                users.map(user => (
                  <tr key={user.id} className="border-b border-outline-variant/30 hover:bg-surface-container-lowest/80 transition-colors duration-150">
                    <td className="px-6 py-4">
                      <div className={`w-9 h-9 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-xs shadow-sm border border-outline-variant/20`}>
                    ...
                    <td className="px-6 py-4 text-right font-bold text-on-surface-variant">{user.setoran} kg</td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex justify-center gap-1">
                        <button onClick={() => handleOpenEditModal(user)} className="w-8 h-8 rounded-md hover:bg-surface-variant text-on-surface-variant flex items-center justify-center transition-colors cursor-pointer" title="Edit">
                          <span className="material-symbols-outlined text-[18px]">edit</span>
                        </button>
                        <button onClick={() => handleDelete(user)} className="w-8 h-8 rounded-md hover:bg-red-50 text-red-600 flex items-center justify-center transition-colors cursor-pointer" title="Hapus">
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-8 text-center text-on-surface-variant font-medium">Tidak ada data pengguna</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-outline-variant/30 flex justify-between items-center bg-surface-container-low">
              <h3 className="text-xl font-bold text-on-surface">{modalType === 'add' ? 'Tambah Pengguna' : 'Edit Pengguna'}</h3>
              <button onClick={handleCloseModal} className="text-on-surface-variant hover:bg-surface-container-low p-2 rounded-full transition-colors cursor-pointer">
            ...
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4 overflow-y-auto max-h-[85vh]">
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-1">NIK (16 Digit)</label>
                <input 
                  type="text" 
                  required 
                  maxLength={16}
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-1">Password {modalType === 'edit' && <span className="text-xs text-on-surface-variant font-normal">(Kosongkan jika tidak diubah)</span>}</label>
                <div className="relative">
                  <input 
              ...
              <div>
                <label className="block text-sm font-medium text-on-surface mb-1">Peran / Role</label>
                <select 
                  required
              ...
              <div className="mt-4 flex justify-end gap-3 pt-4 border-t border-outline-variant/30">
                <button type="button" onClick={handleCloseModal} className="px-4 py-2 rounded-lg font-medium text-on-surface-variant hover:bg-surface-container-low cursor-pointer">Batal</button>
