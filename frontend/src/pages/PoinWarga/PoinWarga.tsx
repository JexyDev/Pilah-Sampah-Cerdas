...
const PoinWarga: React.FC = () => {
  ...
  const medalColor = (rank: number) => {
    if (rank === 1) return { bg: 'bg-yellow-100', color: 'text-yellow-700', border: 'border-yellow-200', medal: 'ðŸ¥‡' };
    if (rank === 2) return { bg: 'bg-gray-100', color: 'text-gray-600', border: 'border-gray-200', medal: 'ðŸ¥ˆ' };
    if (rank === 3) return { bg: 'bg-orange-100', color: 'text-orange-700', border: 'border-orange-200', medal: 'ðŸ¥‰' };
    return { bg: 'bg-surface-container', color: 'text-on-surface', border: 'border-outline-variant/30', medal: '' };
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
        {/* Leaderboard Section */}
        <section className={`bg-white rounded-xl shadow-sm p-5 flex flex-col h-full border border-outline-variant/50 ${selectedUser ? 'xl:col-span-8' : 'xl:col-span-12'}`}>
          ...
          {!searchQuery && !loading && leaders.length >= 3 && (
            <div className="grid grid-cols-3 gap-3 mb-5">
              {[leaders[1], leaders[0], leaders[2]].map((l, idx) => {
                ...
                return (
                  <div key={l.id} className={`flex flex-col items-center p-3 rounded-xl border ${mc.border} ${mc.bg} ${pos === 1 ? 'ring-2 ring-yellow-300 scale-105' : ''} transition-all`}>
                    <span className="text-2xl mb-1">{mc.medal}</span>
                    <p className="text-[13px] font-bold text-center truncate w-full text-center">{l.nama}</p>
                    <p className="text-[10px] text-on-surface-variant">{l.rtRw}</p>
                    <p className={`text-xl font-bold mt-1 ${mc.color}`}>{(l.poin || 0).toLocaleString()}</p>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-9 h-9 rounded-full ${mc.bg} ${mc.color} flex items-center justify-center font-bold text-sm border ${mc.border} shrink-0`}>
                        {leader.rank <= 3 ? mc.medal : `#${leader.rank}`}
                      </div>
                      <div>
                        <p className="text-[14px] font-bold text-on-surface">{leader.nama}</p>
                        <p className="text-[11px] font-bold text-on-surface-variant uppercase tracking-wider mt-0.5">{leader.rtRw || 'RT/RW'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-6 self-end sm:self-auto">
                      <div className="text-right">
                        <p className={`text-xl font-bold ${leader.rank <= 3 ? 'text-primary' : 'text-on-surface'}`}>{(leader.poin || 0).toLocaleString()}</p>
                        <p className="text-[11px] font-bold text-on-surface-variant uppercase tracking-wider">Poin</p>
                      </div>
                      <button
                        onClick={e => { e.stopPropagation(); handleViewDetail(leader); }}
        ...
        {selectedUser && (
          <section className="xl:col-span-4 bg-white rounded-xl shadow-sm border border-outline-variant/50 flex flex-col overflow-hidden sticky top-4">
            <div className="p-4 border-b border-outline-variant/30 flex justify-between items-center bg-surface-container-low/30">
              ...
              <button onClick={() => setSelectedUser(null)} className="text-on-surface-variant hover:text-on-surface p-1 rounded hover:bg-surface-container-low">
            ...
            <div className="p-4 flex flex-col gap-4 overflow-y-auto max-h-[70vh]">
              ...
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-full ${medalColor(selectedUser.rank).bg} ${medalColor(selectedUser.rank).color} flex items-center justify-center text-xl font-bold border ${medalColor(selectedUser.rank).border}`}>
                  {selectedUser.rank <= 3 ? medalColor(selectedUser.rank).medal : selectedUser.nama?.charAt(0)?.toUpperCase()}
                </div>
                <div>
                  ...
                  <p className="text-[11px] font-bold text-primary">Peringkat #{selectedUser.rank}</p>
                </div>
              </div>
              ...
              <div className="bg-green-50 rounded-xl p-4 flex items-center gap-3 border border-green-100">
                ...
                <div>
                  <p className="text-[28px] font-bold text-primary">{(selectedUser.poin || 0).toLocaleString()}</p>
                  <p className="text-[11px] text-on-surface-variant font-bold uppercase tracking-wider">Total Poin Terkumpul</p>
                </div>
              </div>

              {/* Riwayat poin */}
              <div>
                <p className="text-[12px] font-bold text-on-surface-variant uppercase tracking-wider mb-3">Riwayat Poin</p>
                {loadingDetail ? (
                  ...
                  <div className="flex flex-col gap-2">
                    {userDetail.history.slice(0, 15).map((h: any) => (
                      <div key={h.id} className="flex items-center justify-between p-2.5 rounded-lg bg-surface-container-lowest border border-outline-variant/20">
                        <div className="flex items-center gap-2">
                          <span className={`material-symbols-outlined text-[18px] ${h.points >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                            {h.points >= 0 ? 'add_circle' : 'remove_circle'}
                          </span>
                          <div>
                            <p className="text-[12px] font-medium text-on-surface">{h.description || 'Setoran sampah'}</p>
                            <p className="text-[10px] text-on-surface-variant">
                              {h.createdAt ? new Date(h.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : '-'}
                            </p>
                          </div>
                        </div>
                        <span className={`text-[13px] font-bold ${h.points >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {h.points >= 0 ? '+' : ''}{h.points}
                        </span>
                      </div>
                    ))}
