...
export class BinService {
     ...
     * Get all bins
     */
    async getAllBins(currentUser, filters) {
        let whereClause = {};
        if (currentUser) {
     ...
     */
    async processScan(qrCode, userId, householdId, detectedType, estimatedVolume, userLat, userLng, aiConfidence, evidencePhotoUrl, detections) {
        ...
        if (bin.binOwnerships && bin.binOwnerships.length > 0) {
            const isOwner = bin.binOwnerships.some((o) => o.userId === userId);
            if (!isOwner) {
        ...
        if (userLat !== undefined &&
            ...
            const maxRadius = process.env.NODE_ENV === "development" && process.env.ALLOW_GEOFENCE_BYPASS === "true"
                ? 1000000
                : 10;
        ...
        if (detections && detections.length > 0) {
            ...
            for (const det of detections) {
                ...
                if (fullness >= triggerPct) {
                    // Deprecated: dispatchTask is replaced by manual RW notification flow (A.5)
                }
                ...
                const morningStartStr = (await prisma.systemConfig.findUnique({
                    where: { key: "reporting_window_morning_start" },
                }))?.value || "06:00";
                const morningEndStr = (await prisma.systemConfig.findUnique({ where: { key: "reporting_window_morning_end" } }))
                    ?.value || "08:00";
                const eveningStartStr = (await prisma.systemConfig.findUnique({
                    where: { key: "reporting_window_evening_start" },
                }))?.value || "16:00";
                const eveningEndStr = (await prisma.systemConfig.findUnique({ where: { key: "reporting_window_evening_end" } }))
                    ?.value || "18:00";
                const parseTimeToMinutes = (timeStr) => {
                    const [h, m] = timeStr.split(":").map(Number);
                    return h * 60 + m;
                };
                const isWithinMissionWindow = (currentTimeVal >= parseTimeToMinutes(morningStartStr) &&
            ...
            if (userObj?.fcmToken && totalPointsAwarded > 0) {
                await notificationIntegrationService
                    .sendPushNotification(userObj.fcmToken, "Poin Bertambah!", `Selamat! Anda mendapatkan +${totalPointsAwarded} poin dari setoran sampah Anda.`)
        ...
        if (fullness >= 90) {
            await prisma.notification.create({
                data: {
                    ...
                    message: `Kapasitas tempat sampah (${bin.qrCode}) telah mencapai ${Math.round(fullness)}%`,
        ...
        // 6. Convert liters to weight based on density (SRS FR-03: ORGANIC = 0.4 kg/L, NON_ORGANIC = 0.2 kg/L)
        const isOrganic = bin.category.name === "ORGANIC";
        const factor = isOrganic ? DENSITY.ORGANIC : DENSITY.NON_ORGANIC;
        const weightKg = parseFloat((estimatedVolume * factor).toFixed(2));
        // Time-based points calculation with multiplier (SRS FR-03: 100 poin per kg)
        const now = new Date();
        const currentTimeVal = now.getHours() * 60 + now.getMinutes();
        const morningStartStr = (await prisma.systemConfig.findUnique({ where: { key: "reporting_window_morning_start" } }))
            ?.value || "06:00";
        const morningEndStr = (await prisma.systemConfig.findUnique({ where: { key: "reporting_window_morning_end" } }))
            ?.value || "08:00";
        const eveningStartStr = (await prisma.systemConfig.findUnique({ where: { key: "reporting_window_evening_start" } }))
            ?.value || "16:00";
        const eveningEndStr = (await prisma.systemConfig.findUnique({ where: { key: "reporting_window_evening_end" } }))
            ?.value || "18:00";
        const parseTimeToMinutes = (timeStr) => {
        ...
        const pointsPerKg = 100; // SRS FR-03 Standard: 100 points per kg
        const conf = aiConfidence || 1.0;
        let calculatedPoints = Math.round(weightKg * pointsPerKg * multiplier);
        if (conf < 0.9) {
            calculatedPoints = Math.round(calculatedPoints * conf);
        }
        ...
        if (userObj?.fcmToken && calculatedPoints > 0) {
            await notificationIntegrationService
                .sendPushNotification(userObj.fcmToken, "Poin Bertambah!", `Selamat! Anda mendapatkan +${calculatedPoints} poin dari setoran sampah Anda.`)
                .catch((e) => console.error("FCM Error:", e));
        }
        return {
            wasteLogId: result.wasteLog.id,
    ...
    async registerWargaBin(userId, data) {
        ...
        if (codes.length === 0)
            throw new Error("QR_CODES_REQUIRED");
            ...
            for (const qrCode of codes) {
                ...
                await tx.auditTrail.create({
                    data: {
                        ...
                        userId: user.id,
                        oldValue: { qrCode: bin.qrCode, status: bin.status },
                        newValue: { qrCode: bin.qrCode, status: "ACTIVE_BOUND" },
                    },
                });
    ...
    async updateBin(id, data) {
        ...
        if (data.maxCapacityLiter)
            updateData.maxCapacityLiter = parseFloat(data.maxCapacityLiter);
    ...
    async getMyBins(userId) {
        ...
        return bins.map((bin) => {
            ...
            const utamaOwner = bin.binOwnerships?.find((o) => o.type === "UTAMA")?.user;
     ...
     */
    async createResetRequest(binId, userId, evidencePhotoUrl) {
     ...
     */
    async reviewResetRequest(id, status, reviewedById) {
        ...
        if (status === "APPROVED") {
            ...
            // Notify Warga
            await binRepository
                .createNotification(request.userId, "Pengajuan Ditolak", `Pengajuan pengosongan tong ${request.bin.qrCode} ditolak oleh petugas.`)
                .catch(() => { });
        }
        // Log to Audit Trail
     ...
     */
    async assignQrBatch(batchId, picUserId, adminUserId) {
        return binRepository.assignQrBatch(batchId, picUserId, adminUserId);
    }
    /**
     * Mark Bin as Broken
     */
    async markBinAsBroken(qrCode, adminUserId) {
        ...
        return prisma.$transaction(async (tx) => {
            ...
            await tx.auditTrail.create({
                data: {
                    ...
                    userId: adminUserId,
                    oldValue: { id: bin.id, qrCode: bin.qrCode, status: bin.status },
                    newValue: { id: bin.id, qrCode: bin.qrCode, status: "ACTIVE_BOUND" },
                },
            });
            return updatedBin;
        });
    }
    async reactivateBin(binId) {
        const bin = await binRepository.findById(binId);
        if (!bin) {
            throw new Error("BIN_NOT_FOUND");
        }
    ...
    async rejectActivation(binIdOrQrCode, adminUserId) {
        const bin = await prisma.bin.findFirst({
            where: {
                OR: [{ id: binIdOrQrCode }, { qrCode: binIdOrQrCode }],
            },
        ...
        return prisma.$transaction(async (tx) => {
            ...
            await tx.auditTrail.create({
                data: {
                    ...
                    userId: adminUserId,
                    oldValue: { id: bin.id, qrCode: bin.qrCode, status: bin.status },
                    newValue: { id: bin.id, qrCode: bin.qrCode, status: newStatus },
                },
            });
            return updatedBin;
        });
        ...
        if (issueType === "EMPTY_REQUEST") {
            ...
            await prisma.auditTrail
                ...
                data: {
                    ...
                    userId: userId,
                    oldValue: { id: bin.id, qrCode: bin.qrCode, status: bin.status },
                    newValue: { id: bin.id, qrCode: bin.qrCode, status: "BROKEN" },
                },
            })
                .catch(() => { });
    ...
    async updateCapacity(binId, maxCapacityLiter, evidencePhotoUrl) {
        ...
        if (evidencePhotoUrl) {
            await prisma.auditTrail.create({
                data: {
                    ...
                    oldValue: { maxCapacityLiter: Number(bin.maxCapacityLiter) },
                    newValue: { maxCapacityLiter, evidencePhotoUrl },
                },
            });
        }
        return updatedBin;
