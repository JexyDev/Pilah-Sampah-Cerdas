...
export class TransactionService {
    ...
    async createManualDeposit(petugasId, wargaId, beratKg, kategoriId, fotoUrl, overridePoin) {
        return prisma.$transaction(async (tx) => {
            ...
            if (!household)
                throw new Error("Rumah tangga tidak ditemukan untuk warga ini");
            ...
            if (!bin)
                throw new Error("Warga belum memiliki tong sampah aktif");
            const category = await tx.wasteCategory.findUnique({
                where: { id: kategoriId },
            });
            if (!category)
                throw new Error("Kategori sampah tidak valid");
            // 3. Mock AI Check
            // Normally we'd call a real AI service, but we mock the result here for the requirements
            const isOrganic = category.name.toUpperCase().includes("ORGANIK") &&
    ...
    async createResiduDeposit(petugasId, rtRwId, beratKg, photoPath) {
