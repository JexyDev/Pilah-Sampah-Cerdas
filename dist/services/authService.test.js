...
describe("AuthService - registerWarga security", () => {
    ...
    it("should throw PIC_MISMATCH when Mahasiswa KKN registers a bin assigned to another PIC", async () => {
        ...
        await expect(authService.registerWarga({ email: "warga@psc.id", password: "password123", name: "Warga" }, {}, qrCode, wargaSubtype, scannerUser)).rejects.toThrow("PIC_MISMATCH");
    });
    it("should allow registration when Mahasiswa KKN registers a bin assigned to themselves", async () => {
        ...
        const result = await authService.registerWarga({ email: "warga@psc.id", password: "password123", name: "Warga" }, {}, qrCode, wargaSubtype, scannerUser);
