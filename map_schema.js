...
// Replace model maps
for (const [oldT, newT] of Object.entries(tableMap)) {
  schema = schema.replace(new RegExp(`@@map\\("${oldT}"\\)`, 'g'), `@@map("${newT}")`);
}

// Replace column maps
for (const [oldC, newC] of Object.entries(columnMap)) {
  schema = schema.replace(new RegExp(`@map\\("${oldC}"\\)`, 'g'), `@map("${newC}")`);
}

// Special case: field names that need to be mapped if they don't have @map yet, wait, we don't need to add @map if they are already mapped or if we want to change the DB column. Let's just add @map to `email` -> `@map("surel")`, `password` -> `@map("kata_sandi")`, `phone` -> `@map("telepon")`.
schema = schema.replace(/email\s+String\s+@unique/g, 'email           String            @unique @map("surel")');
schema = schema.replace(/password\s+String/g, 'password        String            @map("kata_sandi")');
